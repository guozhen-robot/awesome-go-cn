...
**此项目是 [awesome-go](https://awesome-go.com/) 中文版，最后一次同步时间 : 2019-08-17 13:54:47(每隔1天同步一次)**
...
* [Oto](https://github.com/hajimehoshi/oto) **star:443** 多平台的 low-level 声音播放库。   ![star > 100][Bronze]
* [PortAudio](https://github.com/gordonklaus/portaudio) **star:303** 基于 Go 的PortAudio audio I/O库。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [id3v2](https://github.com/bogem/id3v2) **star:111** 快速稳定的 ID3 解析及写入Go库。   ![star > 100][Bronze]
...
* [jwt-go](https://github.com/dgrijalva/jwt-go) **star:6063** JSON Web令牌(JWT)。   ![star > 5000][Gold]
* [casbin](https://github.com/hsluoyz/casbin) **star:5019** 支持ACL、RBAC、ABAC等访问控制模型的授权库。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [oauth2](https://github.com/golang/oauth2) **star:2393** goauth2的继任者。通用OAuth 2.0包，附带JWT、谷歌api、计算引擎和应用程序引擎支持。   ![star > 1000][Silver]
* [goth](https://github.com/markbates/goth) **star:2292** 提供了 OAuth 和 OAuth2 的简单清晰易用的方法。可开箱即用处理多个提供程序。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [authboss](https://github.com/volatiletech/authboss) **star:1943** web模块化认证系统。它试图删除尽可能多的模板文件和硬编码，以便每次新建一个新的web项目时，您都可以插入、配置并开始构建您的应用程序，而不必每次都构建一个身份验证系统。   ![star > 1000][Silver]
* [osin](https://github.com/openshift/osin) **star:1547** OAuth2服务器库。   ![star > 1000][Silver]
* [go-oauth2-server](https://github.com/RichardKnop/go-oauth2-server) **star:1291** 用 Golang 编写的独立且符合规范的OAuth2服务器。   ![star > 1000][Silver]
* [go-jose](https://github.com/square/go-jose) **star:1141** 相当完整地实现了JOSE工作组的JSON Web令牌、JSON Web签名和JSON Web加密规范。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gologin](https://github.com/dghubble/gologin) **star:1048** 用于使用OAuth1和OAuth2身份验证提供者登录的可链处理程序。   ![star > 1000][Silver]
* [gorbac](https://github.com/mikespook/gorbac) **star:916** 轻量级的基于角色的访问控制(RBAC)实现。   ![star > 100][Bronze]
* [loginsrv](https://github.com/tarent/loginsrv) **star:822** JWT登录微服务带有可插拔的后端服务，如OAuth2 (Github)、htpasswd、osiam。   ![star > 100][Bronze]
* [scs](https://github.com/alexedwards/scs) **star:532** HTTP服务器的会话管理器。   ![star > 100][Bronze]
* [permissions2](https://github.com/xyproto/permissions2) **star:353** 用于跟踪用户、登录状态和权限的库。依赖于cookie安全和bcrypt。   ![star > 100][Bronze]
* [paseto](https://github.com/o1egl/paseto) **star:243** 平台无关的安全令牌(PASETO)。   ![star > 100][Bronze]
...
* [telegram-bot-api](https://github.com/Syfaro/telegram-bot-api) **star:1635** 简单轻量级的Telegram bot客户端。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [telebot](https://github.com/tucnak/telebot) **star:958** 用Go编写的Telegram bot框架。   ![star > 100][Bronze]
* [go-chat-bot](https://github.com/go-chat-bot/bot) **star:471** 用 Go 编写的IRC, Slack和电报机器人。   ![star > 100][Bronze]
* [slacker](https://github.com/shomali11/slacker) **star:317** 可简单创建Slack机器人的框架。   ![star > 100][Bronze]
* [Golang CryptoTrading Bot](https://github.com/saniales/golang-crypto-trading-bot) **star:224** 基于控制台的，用于加密货币交易所的的交易机器人。   ![star > 100][Bronze]
* [tbot](https://github.com/yanzay/tbot) **star:219** 带有类似于net/http API的Telegram bot服务器。   ![star > 100][Bronze]
* [Tenyks](https://github.com/kyleterry/tenyks) **star:168** 面向服务的IRC bot，使用Redis和JSON进行消息传递。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [Kelp](https://github.com/stellar/kelp) **star:165** 官方交易和做市机器人为[Stellar](https://www.stellar.org/) DEX。开箱即用的作品，用 Golang 编写，兼容集中交易和定制交易策略。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [go-sarah](https://github.com/oklahomer/go-sarah) **star:137** 此框架提供了聊天机器人相关的服务，包括LINE、Slack、Gitter等。   ![star > 100][Bronze]
...
* [cobra](https://github.com/spf13/cobra) **star:13487** 现代Go CLI命令行交互工具。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [urfave/cli](https://github.com/urfave/cli) **star:11565** 可让你简单、快速和愉快的构建命令行应用(之前是codegangsta/cli)。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [kingpin](https://github.com/alecthomas/kingpin) **star:2579** 支持子命令的命令行和标志解析器。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-flags](https://github.com/jessevdk/go-flags) **star:1516**  Go 命令行选项解析器。   ![star > 1000][Silver]
* [readline](https://github.com/chzyer/readline) **star:1373** 纯golang实现，在MIT许可下提供了GNU-Readline的大部分特性。   ![star > 1000][Silver]
* [docopt.go](https://github.com/docopt/docopt.go) **star:1175** 会让你满意的命令行参数解析器。   ![star > 1000][Silver]
* [mitchellh/cli](https://github.com/mitchellh/cli) **star:1002** 用于实现命令行接口的Go库。   ![star > 1000][Silver]
* [cli-init](https://github.com/tcnksm/gcli) **star:871** 一个简单就可开启构建Golang命令行的应用程序。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [climax](http://github.com/tucnak/climax)  Alternative CLI with "human face", in spirit of Go command.
* [pflag](https://github.com/spf13/pflag) **star:759** 基于POSIX/GNU-style --flags实现的包，主要用于替换Go的falg包。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [go-arg](https://github.com/alexflint/go-arg) **star:748** 基于结构的参数解析。   ![star > 100][Bronze]
* [complete](https://github.com/posener/complete) **star:625** 使用 Go 语言编写的 bash 命令补全工具以及 Go 命令补全工具.   ![star > 100][Bronze]
* [mow.cli](https://github.com/jawher/mow.cli) **star:624** 用于构建具有复杂标志和参数解析和验证的CLI应用程序。   ![star > 100][Bronze]
* [liner](https://github.com/peterh/liner) **star:592** 类似readline-like的命令行接口库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [cli](https://github.com/mkideal/cli) **star:481** 基于golang结构标签，功能丰富易于使用的命令行包。   ![star > 100][Bronze]
* [flaggy](https://github.com/integrii/flaggy) **star:455** 一个健壮的、易用的标志包，具有出色的子命令支持。   ![star > 100][Bronze]
* [ops](https://github.com/nanovms/ops) **star:269** Unikernel 构建器/协调器。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [argparse](https://github.com/akamensky/argparse) **star:110** 命令行参数分析器，灵感来自Python的argparse模块。   ![star > 100][Bronze]
...
* [sflags](https://github.com/octago/sflags) **star:92** 基于结构的flag生成器，用于flag、urfave/cli、pflag、cobra、kingpin和其他库。
* [wmenu](https://github.com/dixonwille/wmenu) **star:87** 为cli程序提供了简单上手的菜单，可提示用户作出选择。
* [cli](https://github.com/teris-io/cli) **star:57** 为 Go 构建命令接口提供简单而完整的API。
* [job](https://github.com/liujianping/job) **star:52** 工作，把你的短期指令当作长期任务。
...
* [strumt](https://github.com/antham/strumt) **star:27** 用于创建提示链。   ![最近一周有更新][Green]
...
* [go-getoptions](https://github.com/DavidGamba/go-getoptions) **star:10**  Go 选择解析器，借鉴于Perl灵活性的GetOpt::Long。   ![最近一周有更新][Green]
...
* [termui](https://github.com/gizak/termui) **star:8965** 此库是基于**termbox-go**实现的，借鉴于[blessed-contrib](https://github.com/yaronn/blessed-contrib)。   ![star > 5000][Gold]
* [gommon/color](https://github.com/labstack/gommon/tree/master/color)  更换终端文本样式。
* [gocui](https://github.com/jroimartin/gocui) **star:5426** 旨在创建控制台用户界面的极简Go库。   ![star > 5000][Gold]
* [termbox-go](https://github.com/nsf/termbox-go) **star:3497** 基于文本的跨平台接口库。   ![star > 1000][Silver]
* [color](https://github.com/fatih/color) **star:3033** 多功能包装，彩色终端输出。   ![star > 1000][Silver]   ![归档项目][Archived]
* [go-prompt](https://github.com/c-bata/go-prompt) **star:2354** 构建一个强大的交互式提示，借鉴于[python-prompt-toolkit](https://github.com/jonathanslenders/python-prompt-toolkit)   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [uiprogress](https://github.com/gosuri/uiprogress) **star:1544** 在终端呈现进度条，可灵活配置的。   ![star > 1000][Silver]
* [asciigraph](https://github.com/guptarohit/asciigraph) **star:1150** 在命令行中构建轻量级ASCII线图╭┈╯，应用程序中没有其他依赖项。   ![star > 1000][Silver]
* [uilive](https://github.com/gosuri/uilive) **star:837** 用于实时更新终端输出的库。   ![star > 100][Bronze]
...
* [termdash](https://github.com/mum4k/termdash) **star:816** 此库是基于**termbox-go**实现的，借鉴于[termui](https://github.com/gizak/termui)。   ![star > 100][Bronze]
* [mpb](https://github.com/vbauerster/mpb) **star:714** 可在终端显示多进度条。   ![star > 100][Bronze]
* [aurora](https://github.com/logrusorgru/aurora) **star:638** 支持fmt.Printf/Sprintf的ANSI终端颜色。   ![star > 100][Bronze]
* [progressbar](https://github.com/schollz/progressbar) **star:581** 基本线程安全的进度条，在每个操作系统工作。   ![star > 100][Bronze]
* [uitable](https://github.com/gosuri/uitable) **star:505** 改善终端应用程序中表格数据的可读性。   ![star > 100][Bronze]
* [go-colorable](https://github.com/mattn/go-colorable) **star:379** 适用于windows的颜色编写器。   ![star > 100][Bronze]
* [go-isatty](https://github.com/mattn/go-isatty) **star:346** Go 实现的 isatty。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [chalk](https://github.com/ttacon/chalk) **star:307** 美化终端/控制台输出。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [tabby](https://github.com/cheynewallace/tabby) **star:249** 一个可在终端生成一个极简Golang表格轻量级库   ![star > 100][Bronze]
* [gookit/color](https://github.com/gookit/color) **star:210** 终端显色工具库，支持16种颜色，256种颜色，RGB显色输出，兼容Windows。   ![star > 100][Bronze]   ![最近一周有更新][Green]   ![包含中文文档][CN]
...
* [viper](https://github.com/spf13/viper) **star:9439** 配置管理。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [kelseyhightower/envconfig](https://github.com/kelseyhightower/envconfig) **star:2419** 管理来自环境变量的配置数据。   ![star > 1000][Silver]
* [godotenv](https://github.com/joho/godotenv) **star:2147** Ruby 的 dotenv 库的 Go移植版(从.env文件加载环境变量)。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [ini](https://github.com/go-ini/ini) **star:1608**  读和写INI文件。   ![star > 1000][Silver]
* [env](https://github.com/caarlos0/env) **star:879** 解析环境变量并赋值到struct中(默认值)。   ![star > 100][Bronze]
* [konfig](https://github.com/lalamove/konfig) **star:515** 可组合、可观察和高性能的分布式配置管理。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [config](https://github.com/JeremyLoy/config) **star:189** 云本地应用程序配置。将ENV绑定到结构体仅需两行代码。   ![star > 100][Bronze]
* [hjson](https://github.com/hjson/hjson-go) **star:172** 更加人性化的JSON配置。轻松的语法，更少的错误，更多的注释。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [envconfig](https://github.com/vrischmann/envconfig) **star:146** 从环境变量中读取配置。   ![star > 100][Bronze]
...
* [envh](https://github.com/antham/envh) **star:92** 协助管理环境变量的Helpers。   ![最近一周有更新][Green]
...
* [gookit/config](https://github.com/gookit/config) **star:87** 程序配置管理(load,get,set)。支持JSON, YAML, TOML, INI, HCL。支持多文件加载，数据覆盖合并。   ![包含中文文档][CN]
...
* [drone](https://github.com/drone/drone) **star:19117** Drone 是一个基于 Docker 的持续集成平台，用 Go 编写。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [goveralls](https://github.com/mattn/goveralls) **star:580** Coveralls.io 是一个用 Go 编写，可持续对代码覆盖率进行检测的系统。   ![star > 100][Bronze]
...
* [gods](https://github.com/emirpasic/gods) **star:6455** 数据结构。容器、集合、列表、堆栈、地图、BidiMaps、树、HashSet等。   ![star > 5000][Gold]
* [go-datastructures](https://github.com/Workiva/go-datastructures) **star:5148** 可靠的、高性能的和线程安全的数据结构的集合。   ![star > 5000][Gold]
* [golang-set](https://github.com/deckarep/golang-set) **star:1185** 线程安全和非线程安全的高性能集。   ![star > 1000][Silver]
* [boomfilters](https://github.com/tylertreat/BoomFilters) **star:1164** 用于处理连续的概率数据结构。   ![star > 1000][Silver]
* [gota](https://github.com/kniren/gota) **star:893** 实现了数据帧，序列以及数据噪音。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [willf/bloom](https://github.com/willf/bloom) **star:670** 实现Bloom过滤器。   ![star > 100][Bronze]
* [roaring](https://github.com/RoaringBitmap/roaring) **star:670** 实现了压缩 bitsets 的Go包。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [cuckoofilter](https://github.com/seiflotfy/cuckoofilter) **star:518** 布谷鸟过滤器:一个用Go实现，可替代计数 bloom 过滤器。   ![star > 100][Bronze]
* [bitset](https://github.com/willf/bitset) **star:482** 实现了 bitsets 的 Go 包。   ![star > 100][Bronze]
* [trie](https://github.com/derekparker/trie) **star:425** 在Go中实现Trie。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [algorithms](https://github.com/shady831213/algorithms) **star:247** 算法和数据结构。来源于CLRS。   ![star > 100][Bronze]
* [goskiplist](https://github.com/ryszard/goskiplist) **star:195** 基于 Go 的跳表实现。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [hilbert](https://github.com/google/hilbert) **star:184** 用于映射空间填充曲线（例如 Hilbert 曲线和 Peano 曲线）和数值。   ![star > 100][Bronze]
...
* [binpacker](https://github.com/zhuangsirui/binpacker) **star:128** 帮助用户构建自定义二进制流的二进制封装器和解包器   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [deque](https://github.com/gammazero/deque) **star:69** 快速环缓冲区deque(双端队列)。
* [skiplist](https://github.com/gansidui/skiplist) **star:65** 在Go中实现了跳表。   ![最近一年没有更新][Yellow]
* [levenshtein](https://github.com/agnivade/levenshtein) **star:56** 实现在Go中计算levenshtein距离。
...
* [bloom](https://github.com/yourbasic/bloom) **star:40** Golang Bloom过滤器的实现。   ![最近一年没有更新][Yellow]
* [go-mcache](https://github.com/OrlovEvgeny/go-mcache) **star:36** 基于内存的实现了高性能的key:value存储库。指针缓存。
...
* [prometheus](https://github.com/prometheus/prometheus) **star:25795** 用于监控系统和时序的数据库。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [tidb](https://github.com/pingcap/tidb) **star:20196** TiDB是一个分布式SQL数据库。灵感来自谷歌F1的设计。   ![star > 5000][Gold]   ![最近一周有更新][Green]   ![包含中文文档][CN]
* [influxdb](https://github.com/influxdb/influxdb) **star:17173** 可伸缩的数据存储，用于指标衡量、事件和实时分析。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [cockroach](https://github.com/cockroachdb/cockroach) **star:16826** 可伸缩、区域备份、事务性数据存储。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [dgraph](https://github.com/dgraph-io/dgraph) **star:10545** 可伸缩、分布式、低延迟、高吞吐量的图形数据库。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [bolt](https://github.com/boltdb/bolt) **star:10015** K/V 数据库。   ![star > 5000][Gold]   ![最近一年没有更新][Yellow]   ![归档项目][Archived]
* [groupcache](https://github.com/golang/groupcache) **star:7683** Groupcache是一个缓存和缓存填充库，在许多情况下，它是memcached的替代品。   ![star > 5000][Gold]
* [badger](https://github.com/dgraph-io/badger) **star:6314** 快速 K/V 存储。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [rqlite](https://github.com/rqlite/rqlite) **star:4711** 基于SQLite的轻量级分布式关系数据库。   ![star > 1000][Silver]
* [goleveldb](https://github.com/syndtr/goleveldb) **star:3176** 在Go中实现[LevelDB](https://github.com/google/leveldb) key/value数据库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [ledisdb](https://github.com/siddontang/ledisdb) **star:3085** Ledisdb是一种高性能的NoSQL，类似于基于LevelDB的Redis。   ![star > 1000][Silver]
* [go-cache](https://github.com/pmylund/go-cache) **star:2931** 基于内存的 K/V 存储/缓存 : (类似于Memcached)，适用于单机应用程序。   ![star > 1000][Silver]
* [BigCache](https://github.com/allegro/bigcache) **star:2471** 高效的键/值缓存为千兆字节的数据。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [buntdb](https://github.com/tidwall/buntdb) **star:2448** 基于内存的K/V，快速，可嵌入的数据库，可自定义索引和空间支持。   ![star > 1000][Silver]
* [tiedot](https://github.com/HouzuoGuo/tiedot) **star:2370** 属于你的NoSQL数据库。   ![star > 1000][Silver]
* [cache2go](https://github.com/muesli/cache2go) **star:1056** 基于内存的 K/V 缓存，支持超时的自动失效。   ![star > 1000][Silver]
* [VictoriaMetrics](https://github.com/VictoriaMetrics/VictoriaMetrics) **star:1042** 开源，快速，可伸缩的时间序列数据库。支持PromQL。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [GCache](https://github.com/bluele/gcache) **star:909** 支持过期缓存、LFU、LRU和ARC的缓存库。   ![star > 100][Bronze]
* [nutsdb](https://github.com/xujiajun/nutsdb) **star:893** Nutsdb是一个用纯Go编写的简单、快速、可嵌入、持久的键/值存储。它支持完全序列化的事务和许多数据结构，如列表、集合、排序集。   ![star > 100][Bronze]   ![包含中文文档][CN]
* [CovenantSQL](https://github.com/CovenantSQL/CovenantSQL) **star:879** 区块链领域的一个SQL数据库。   ![star > 100][Bronze]
* [diskv](https://github.com/peterbourgon/diskv) **star:756** 支持磁盘备份的可持久化 K/V 存储。   ![star > 100][Bronze]
...
* [fastcache](https://github.com/VictoriaMetrics/fastcache) **star:497** 基于内存的快速线程安全的缓存，可缓存大量的条目。最大限度地减少GC开销。   ![star > 100][Bronze]
* [levigo](https://github.com/jmhodges/levigo) **star:364** 实现了对LevelDB封装。   ![star > 100][Bronze]
* [pudge](https://github.com/recoilme/pudge) **star:219** 使用Go的标准库编写的快速和简单的键/值存储。   ![star > 100][Bronze]
...
* [Scribble](https://github.com/nanobox-io/golang-scribble) **star:62** 小型平面文件JSON存储。
...
* [cache](https://github.com/akyoto/cache) **star:17** 基于内存的 K/V 存储:带生命周期的值存储，0个依赖项，<100 LoC, 100%覆盖率。
...
* [migrate](https://github.com/golang-migrate/migrate) **star:2797** 基于CLI的数据库迁移库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [sql-migrate](https://github.com/rubenv/sql-migrate) **star:1416** 数据库迁移工具。允许使用go-bindata将迁移嵌入到应用程序中。   ![star > 1000][Silver]
* [gormigrate](https://github.com/go-gormigrate/gormigrate) **star:337** 面向Gorm ORM的数据库 schema 迁移辅助程序。   ![star > 100][Bronze]
...
* [migrator](https://github.com/lopezator/migrator) **star:33** 非常简单的 Go 数据库迁移库。   ![最近一周有更新][Green]
...
* [vitess](https://github.com/youtube/vitess) **star:8491** vitess提供了可以为大规模web服务扩展MySQL数据库提供便利的服务和工具。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [pgweb](https://github.com/sosedoff/pgweb) **star:6002** 基于web的PostgreSQL数据库浏览器。   ![star > 5000][Gold]
* [kingshard](https://github.com/flike/kingshard) **star:4639** kingshard 是基于 Golang 的MySQL高性能代理。   ![star > 1000][Silver]   ![最近一周有更新][Green]   ![包含中文文档][CN]
* [orchestrator](https://github.com/github/orchestrator) **star:3063** MySQL复制拓扑管理器和可视化工具。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-mysql-elasticsearch](https://github.com/siddontang/go-mysql-elasticsearch) **star:2420** 自动将MySQL数据同步到Elasticsearch中。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [pREST](https://github.com/nuveo/prest) **star:2095** 基于PostgreSQL database的RESTful API服务。   ![star > 1000][Silver]
* [go-mysql](https://github.com/siddontang/go-mysql) **star:1903**  Go 工具集，用于处理MySQL协议和复制。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [chproxy](https://github.com/Vertamedia/chproxy) **star:310** ClickHouse数据库的HTTP代理。   ![star > 100][Bronze]
...
* [octillery](https://github.com/knocknote/octillery) **star:55** 用于数据库分表(支持每个ORM或原生SQL)。
...
* [Squirrel](https://github.com/Masterminds/squirrel) **star:2340** 帮助您构建SQL查询的Go库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [xo](https://github.com/knq/xo) **star:2191** 基于现有的schema定义和自定义查询生成 Go 代码，基于支持PostgreSQL、MySQL、SQLite、Oracle和Microsoft SQL Server。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gendry](https://github.com/didi/gendry) **star:770** 非入侵的SQL构建器和强大的数据绑定器。   ![star > 100][Bronze]   ![包含中文文档][CN]
...
* Relational Databases
    * [go-sql-driver/mysql](https://github.com/go-sql-driver/mysql) **star:8149** MySQL驱动程序。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [pq](https://github.com/lib/pq) **star:5211** 纯 Go 的Postgres驱动。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [go-sqlite3](https://github.com/mattn/go-sqlite3) **star:3442** SQLite3驱动程序。   ![star > 1000][Silver]
    * [pgx](https://github.com/jackc/pgx) **star:1971** PostgreSQL驱动，支持比现有database/sql更多的特性。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [go-mssqldb](https://github.com/denisenkom/go-mssqldb) **star:1025** 微软MSSQL驱动程序。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [go-oci8](https://github.com/mattn/go-oci8) **star:408** Oracle 驱动程序。   ![star > 100][Bronze]
    ...
    * [gofreetds](https://github.com/minus5/gofreetds) **star:91** 基于[FreeTDS](http://www.freetds.org)封装的微软MSSQL Go 驱动。
...
* NoSQL Databases
    * [redis](https://github.com/go-redis/redis) **star:6603** 基于 Go 的 Redis 客户端。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [redigo](https://github.com/gomodule/redigo) **star:6337** Redigo 是基于 Go 的Redis 客户端。   ![star > 5000][Gold]
    * [mongo-go-driver](https://github.com/mongodb/mongo-go-driver) **star:3170** 官方的 MongoDB 驱动。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [mgo](https://github.com/globalsign/mgo) **star:1651** (已停止维护) MongoDB驱动。   ![star > 1000][Silver]
    * [gorethink](https://github.com/dancannon/gorethink) **star:1463** RethinkDB 驱动。   ![star > 1000][Silver]
    * [neoism](https://github.com/jmcvetta/neoism) **star:357** Golang 的 Neo4j 客户端。   ![star > 100][Bronze]
    * [aerospike-client-go](https://github.com/aerospike/aerospike-client-go) **star:306** Aerospike 客户端。   ![star > 100][Bronze]   ![最近一周有更新][Green]
    ...
    * [redeo](https://github.com/bsm/redeo) **star:262** 与 redis 协议兼容的 TCP 服务器/服务。   ![star > 100][Bronze]   ![最近一周有更新][Green]
    ...
    * [godis](https://github.com/piaohao/godis) **star:62** 由GoLang实现的redis客户端，灵感来自jedis。   ![最近一周有更新][Green]
    * [go-couchdb](https://github.com/fjl/go-couchdb) **star:51** 基于 Go 的 Yet another CouchDB 的 Http 接口封装。
    * [go-pilosa](https://github.com/pilosa/go-pilosa) **star:31**  Pilosa客户端。   ![最近一周有更新][Green]
...
* Search and Analytic Databases.
    * [bleve](https://github.com/blevesearch/bleve) **star:5866** 基于 Go 的现代文本索引库。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [riot](https://github.com/go-ego/riot) **star:4710** 基于 Go 的 开源、分布式、简单高效的搜索引擎。   ![star > 1000][Silver]   ![包含中文文档][CN]
    * [elastic](https://github.com/olivere/elastic) **star:4178** Elasticsearch 客户端。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [go-elasticsearch](https://github.com/elastic/go-elasticsearch) **star:1611** 官方 Elasticsearch 客户端。   ![star > 1000][Silver]
    * [elastigo](https://github.com/mattbaird/elastigo) **star:951** Elasticsearch 客户端。   ![star > 100][Bronze]
    * [elasticsql](https://github.com/cch123/elasticsql) **star:402** 将 SQL 转换为 elasticsearch dsl。   ![star > 100][Bronze]
...
* Multiple Backends.
    * [cayley](https://github.com/google/cayley) **star:12699** 图形数据库，支持多个后端。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [cachego](https://github.com/fabiorphp/cachego) **star:110** 基于多个驱动程序的缓存组件。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [now](https://github.com/jinzhu/now) **star:2193** now 是时间有关的工具类。   ![star > 1000][Silver]
* [dateparse](https://github.com/araddon/dateparse) **star:901** 可以解析很多格式不固定的日期字符串。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [carbon](https://github.com/uniplaces/carbon) **star:341** 简单的时间扩展，包含了许多使用方法，从 PHP Carbon 库移植的。   ![star > 100][Bronze]
* [durafmt](https://github.com/hako/durafmt) **star:243** 轻量级、可让time.Duration更加易读的库。   ![star > 100][Bronze]
...
* [timespan](https://github.com/SaidinWoT/timespan) **star:61** 用于处理时间间隔相关的库，可定义开始时间和持续时间。
...
* [go-kit](https://github.com/go-kit/kit) **star:14538** 支持服务发现、负载平衡、插件式传输、请求跟踪等功能的Microservice toolkit。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [grpc-go](https://github.com/grpc/grpc-go) **star:9193** gRPC的Go语言实现。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [micro](https://github.com/micro/micro) **star:6623** 可插拔的微服务 toolkit 和分布式系统平台。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [NATS](https://github.com/nats-io/gnatsd) **star:6382** 轻量级、高性能消息传递系统，可用于微服务、物联网(IoT)和云。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [rpcx](https://github.com/smallnest/rpcx) **star:3832** 分布式可插拔的RPC服务框架，如阿里巴巴Dubbo。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [tendermint](https://github.com/tendermint/tendermint) **star:3167** 一个高性能中间件，可将任何语言的状态机转换为 Byzantine Fault 状态机。使用 Tendermint 一致性及区块链协议。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [torrent](https://github.com/anacrolix/torrent) **star:2879** BitTorrent 客户端。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [raft](https://github.com/coreos/etcd/tree/master/raft)  Raft consensus协议的实现。 by CoreOS。
* [raft](https://github.com/hashicorp/raft) **star:2857** Raft consensus协议的实现。 by HashiCorp。   ![star > 1000][Silver]
* [dragonboat](https://github.com/lni/dragonboat) **star:2564** 一个功能齐全，高性能的库集。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [glow](https://github.com/chrislusf/glow) **star:2538** 全部用 Go 实现，易用、可伸缩，可用于分布式大数据处理，Map-Reduce, DAG执行。   ![star > 1000][Silver]
...
* [emitter-io](https://github.com/emitter-io/emitter) **star:1944** 高性能、分布式、安全和低延迟的发布-订阅平台，使用MQTT、Websockets和love构建。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [KrakenD](https://github.com/devopsfaith/krakend) **star:1775** 具有中间件的高性能API网关框架。   ![star > 1000][Silver]
* [hprose](https://github.com/hprose/hprose-golang) **star:1012** 支持25+种语言RPC库。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [ringpop-go](https://github.com/uber/ringpop-go) **star:576** 可伸缩的，容错、应用分层的的Go应用程序。   ![star > 100][Bronze]
* [gorpc](https://github.com/valyala/gorpc) **star:554** 简单、快速和可伸缩的RPC库。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [go-health](https://github.com/InVisionApp/go-health) **star:484** 用于在服务中启用异步依赖项健康检查的库。   ![star > 100][Bronze]
...
* [redis-lock](https://github.com/bsm/redis-lock) **star:146** 基于redis的分布式锁简易实现。   ![star > 100][Bronze]   ![归档项目][Archived]
* [dht](https://github.com/anacrolix/dht) **star:130** BitTorrent Kademlia DHT的实现。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [celeriac](https://github.com/svcavallar/celeriac.v1) **star:53** 用于对 Celery worker、任务、事件进行交互和监控的库。
...
* [MailHog](https://github.com/mailhog/MailHog) **star:5195** 电子邮件和SMTP测试工具，对外提供了 web 和 API 接口。   ![star > 5000][Gold]
* [chasquid](https://blitiri.com.ar/p/chasquid)  用Go编写的SMTP服务器。
* [hermes](https://github.com/matcornic/hermes) **star:1625** 可生成干净的、响应式的HTML电子邮件。   ![star > 1000][Silver]
* [email](https://github.com/jordan-wright/email) **star:1100** 一个强大和灵活的电子邮件库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-imap](https://github.com/emersion/go-imap) **star:737** 用于客户端和服务器的IMAP库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [SendGrid](https://github.com/sendgrid/sendgrid-go) **star:521** SendGrid 的 Go语言库，用于发送电子邮件。   ![star > 100][Bronze]
* [Hectane](https://github.com/hectane/hectane) **star:169** 轻量级的SMTP客户机，提供了HTTP API。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [douceur](https://github.com/aymerick/douceur) **star:162** 在HTML邮件中支持CSS内联。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [go-message](https://github.com/emersion/go-message) **star:115** 用于Internet消息格式化和邮件消息的流媒体库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [otto](https://github.com/robertkrimen/otto) **star:4753** 用 Go 编写的 JavaScript 解释器。   ![star > 1000][Silver]
* [gopher-lua](https://github.com/yuin/gopher-lua) **star:2971** 用 Go 实现的 Lua 5.1 虚拟机和编译器。   ![star > 1000][Silver]
* [go-lua](https://github.com/Shopify/go-lua) **star:1676** 用 Go 实现的 Lua 5.2 VM接口。   ![star > 1000][Silver]
* [tengo](https://github.com/d5/tengo) **star:1320** 字节码编译的脚本语言。   ![star > 1000][Silver]
* [anko](https://github.com/mattn/anko) **star:923** 用Go编写的解释器。   ![star > 100][Bronze]
* [go-python](https://github.com/sbinet/go-python) **star:910** CPython C-API 的 Go 接口。   ![star > 100][Bronze]
* [expr](https://github.com/antonmedv/expr) **star:709** 一个可以计算表达式的引擎。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [go-php](https://github.com/deuill/go-php) **star:686** PHP 的 Go 接口。   ![star > 100][Bronze]
* [go-duktape](https://github.com/olebedev/go-duktape) **star:654** 支持 Duktape JavaScript 引擎。   ![star > 100][Bronze]
...
* [errors](https://github.com/pkg/errors) **star:4926** 可让你很简单的进行错误处理。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-multierror](https://github.com/hashicorp/go-multierror) **star:734** 可将一系列的错误作为一个整体来显示。   ![star > 100][Bronze]
* [errorx](https://github.com/joomcode/errorx) **star:565** 一个功能丰富的错误包，可进行堆栈跟踪、组装异常信息以及其他。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [tracerr](https://github.com/ztrue/tracerr) **star:497** 可展示错误的堆栈跟踪信息和源码片段。   ![star > 100][Bronze]
* [errlog](https://github.com/snwfdhmp/errlog) **star:190** 用于定位抛出错误的源代码(以及一些其他快速调试特性)。可插入到任何 logger 的位置。   ![star > 100][Bronze]
...
* [afero](https://github.com/spf13/afero) **star:2256** 文件系统的抽象系统。   ![star > 1000][Silver]
* [pdfcpu](https://github.com/hhrutter/pdfcpu) **star:956** PDF处理器。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [notify](https://github.com/rjeczalik/notify) **star:497** 文件系统事件通知库，具有类似于os/signal的简单API，。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [vfs](https://github.com/C2FO/vfs) **star:25** 一组可插拔的、可扩展的和自定义的文件系统功能，用于跨越许多文件系统类型，如os、S3和GCS。
...
* [checksum](https://github.com/codingsince1985/checksum) **star:7** 生成大型文件的消息摘要(如 MD5 和 SHA256)。
* [go-exiftool](https://github.com/barasher/go-exiftool) **star:5** ExifTool 的 Go 实现，这个著名的库用于从文件(图片、PDF、office，…)中提取尽可能多的元数据(EXIF、IPTC，…)。

## 金融

*会计和财务软件包。 (翻译出错了? 试试 [英文版](README_EN.md#financial) 吧~)*

* [decimal](https://github.com/shopspring/decimal) **star:1619** 任意精度定点的十进制数。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-money](https://github.com/rhymond/go-money) **star:624** Fowler 货币模式的实现。   ![star > 100][Bronze]
* [go-finance](https://github.com/FlashBoys/go-finance) **star:537** 综合金融市场数据。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [accounting](https://github.com/leekchan/accounting) **star:489** 货币和货币格式。   ![star > 100][Bronze]
...
* [transaction](https://github.com/claygod/transaction) **star:56** 嵌入式事务数据库，可多线程模式运行。
...
* [nosurf](https://github.com/justinas/nosurf) **star:974** CSRF保护中间件。   ![star > 100][Bronze]
* [binding](https://github.com/mholt/binding) **star:754** 将来自 net/HTTP 请求的表单、JSON 数据绑定到结构体。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [gorilla/csrf](https://github.com/gorilla/csrf) **star:441** 用于Go web应用程序和服务的CSRF保护。   ![star > 100][Bronze]
...
* [conform](https://github.com/leebenson/conform) **star:174** 控制用户输入。基于struct tags可对数据进行修剪、清理和擦除。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [formam](https://github.com/monoculum/formam) **star:129** 将表单的值解码为 struct。   ![star > 100][Bronze]
...
* [fuego](https://github.com/seborama/fuego) **star:41** Functional Experiment in Go。   ![最近一周有更新][Green]

## 游戏开发

*很棒的游戏开发库。 (翻译出错了? 试试 [英文版](README_EN.md#game-development) 吧~)*

* [Leaf](https://github.com/name5566/leaf) **star:3076** 轻量级游戏服务器框架。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [Pixel](https://github.com/faiface/pixel) **star:2457** 手工制作的 2D 游戏库。   ![star > 1000][Silver]
* [Ebiten](https://github.com/hajimehoshi/ebiten) **star:1869** 很简单的 2D 游戏库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [goworld](https://github.com/xiaonanln/goworld) **star:1207** 可伸缩的游戏服务器引擎，具有 space-entity 框架和 hot-swapping 功能。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [go-sdl2](https://github.com/veandco/go-sdl2) **star:1161** 实现了[Simple DirectMedia Layer](https://www.libsdl.org/)。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [engo](https://github.com/EngoEngine/engo) **star:1087** 开源 2D 游戏引擎。它遵循 Entity-Component-System 范式。   ![star > 1000][Silver]
* [gonet](https://github.com/xtaci/gonet) **star:1052** 实现了游戏服务器骨架。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [termloop](https://github.com/JoelOtter/termloop) **star:1028** 基于终端的 Go 游戏引擎，建立在 Termbox 之上。   ![star > 1000][Silver]
* [nano](https://github.com/lonng/nano) **star:1004** 轻量级、方便、高性能的基于golang的游戏服务器框架。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [g3n](https://github.com/g3n/engine) **star:760**  3D游戏引擎。   ![star > 100][Bronze]
* [Oak](https://github.com/oakmound/oak) **star:630** 纯 Go 实现的游戏引擎。   ![star > 100][Bronze]
* [Azul3D](https://github.com/azul3d/engine) **star:426** 用Go编写的 3D 游戏引擎。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [raylib-go](https://github.com/gen2brain/raylib-go) **star:385** 实现了 [raylib](http://www.raylib.com/)，一个简单易用的库，用于学习视频游戏编程。   ![star > 100][Bronze]
* [go-astar](https://github.com/beefsack/go-astar) **star:329** 实现了A\*路径查找算法。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [GarageEngine](https://github.com/vova616/GarageEngine) **star:313** 用 OpenGL 编写的 2D 游戏引擎。   ![star > 100][Bronze]
* [Pitaya](https://github.com/topfreegames/pitaya) **star:302** 可伸缩的游戏服务器框架，支持集群和客户端库的iOS, Android, Unity。   ![star > 100][Bronze]
...
* [go-linq](https://github.com/ahmetalpbalkan/go-linq) **star:1801** 提供类似 .NET LINQ 的查询方法。   ![star > 1000][Silver]
* [jennifer](https://github.com/dave/jennifer) **star:1285** 不使用模板生成任意 Go 代码。   ![star > 1000][Silver]
* [gen](https://github.com/clipperhouse/gen) **star:1039** 用于生成泛型等类似方法的功能代码生成工具。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [goderive](https://github.com/awalterschulze/goderive) **star:748** 从输入类型来派生函数。   ![star > 100][Bronze]
* [GoWrap](https://github.com/hexdigest/gowrap) **star:268** 使用简单模板为 Go 接口生成装饰器。   ![star > 100][Bronze]
...
* [S2 geometry](https://github.com/golang/geo) **star:895** S2 geometry 库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [gopherjs](https://github.com/gopherjs/gopherjs) **star:8572** 将 Go 编译成 JavaScript。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [llgo](https://github.com/go-llvm/llgo) **star:989** 基于 llvm 的编译器。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [c4go](https://github.com/Konstantin8105/c4go) **star:158** 将 C 代码转换为 Go 代码。   ![star > 100][Bronze]
...
* [goworker](https://github.com/benmanns/goworker) **star:2259** 基于 go 的后台 worker。   ![star > 1000][Silver]
* [ants](https://github.com/panjf2000/ants) **star:1919** 一个高性能的协程池。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [tunny](https://github.com/Jeffail/tunny) **star:1360** golang 的协程池。   ![star > 1000][Silver]
* [grpool](https://github.com/ivpusic/grpool) **star:503** 轻量级协程池。   ![star > 100][Bronze]
...
* [workerpool](https://github.com/gammazero/workerpool) **star:139** 限制任务执行并发数，而不是队列中的任务数量的协程池，。   ![star > 100][Bronze]
* [go-flow](https://github.com/kamildrazkiewicz/go-flow) **star:105** 控制 goroutines 的执行顺序。   ![star > 100][Bronze]
* [GoSlaves](https://github.com/themester/GoSlaves) **star:76** 简单异步的协程池。
* [semaphore](https://github.com/kamilsk/semaphore) **star:75** 信号量模式实现，可根据通道和上下文进行具备超时功能的锁定/解锁操作。
...
* [worker-pool](https://github.com/vardius/worker-pool) **star:46** 一个简单的 Go 异步工作池。
* [cyclicbarrier](https://github.com/marusama/cyclicbarrier) **star:36** 基于 Go 的 CyclicBarrier 实现。
* [breaker](https://github.com/kamilsk/breaker) **star:34** 灵活的机制，可以使执行流可中断。
...
* [Hunch](https://github.com/AaronJan/Hunch) **star:15** Hunch 提供了诸如 All、First、Retry、Waterfall 等功能，这使得异步流控制更加直观。
...
* [ui](https://github.com/andlabs/ui) **star:7002** 跨平台的 Platform-native GUI 库。   ![star > 5000][Gold]
* [Wails](https://wails.app)  Mac, Windows, Linux桌面应用程序，主要基于含有内置的OS HTML渲染器的HTML UI。
* [fyne](https://github.com/fyne-io/fyne) **star:6355** 为 Go 而设计的跨平台的本地GUIs，使用EFL呈现。支持 : Linux, macOS, Windows。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [qt](https://github.com/therecipe/qt) **star:6102** 实现了 Qt 的 Go接口(支持Windows / macOS / Linux / Android / iOS / Sailfish OS / Raspberry Pi)。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [webview](https://github.com/zserge/webview) **star:4688** 跨平台webview窗口，具有简单的双向JavaScript绑定(Windows / macOS / Linux)。   ![star > 1000][Silver]
* [walk](https://github.com/lxn/walk) **star:3714** Windows应用程序库。   ![star > 1000][Silver]
* [app](https://github.com/murlokswarm/app) **star:2964** 用于创建包含了 GO, HTML 和 CSS 的应用程序。支持 MacOS, Windows 正在开发中。   ![star > 1000][Silver]
* [go-astilectron](https://github.com/asticode/go-astilectron) **star:2689** 使用 GO 和 HTML/JS/CSS (电子驱动)进行构建跨平台 GUI 应用程序。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-gtk](http://mattn.github.io/go-gtk/)  实现了 GTK 的 Go接口。
* [go-sciter](https://github.com/sciter-sdk/go-sciter) **star:1453** 实现了 Sciter 的 Go 接口 : 用于现代桌面 UI 开发的可嵌入HTML/CSS/脚本引擎。可跨平台。   ![star > 1000][Silver]
* [gotk3](https://github.com/gotk3/gotk3) **star:775** 实现了 GTK3 的 Go接口。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [gowd](https://github.com/dtylman/gowd) **star:213** 跨平台、快速、简单的桌面UI开发，采用了GO, HTML, CSS和NW.js实现。   ![star > 100][Bronze]

*交互 (翻译出错了? 试试 [英文版](README_EN.md#gui) 吧~)*

* [robotgo](https://github.com/go-vgo/robotgo) **star:4443** 实现跨平台的GUI系统自动化。包含了控制鼠标、键盘等功能。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [systray](https://github.com/getlantern/systray) **star:793** 跨平台 Go 库，可在通知区放置图标和菜单。   ![star > 100][Bronze]
* [gosx-notifier](https://github.com/deckarep/gosx-notifier) **star:496** OSX 桌面通知库。   ![star > 100][Bronze]
...
* [mac-sleep-notifier](https://github.com/prashantgupta24/mac-sleep-notifier) **star:1** OSX 睡眠/唤醒通知。
...
* [imaginary](https://github.com/h2non/imaginary) **star:2596** 用于图像大小调整的快速、简单的HTTP微服务。   ![star > 1000][Silver]
* [bild](https://github.com/anthonynsimon/bild) **star:2555** 纯Go语言实现的图像处理算法合集。   ![star > 1000][Silver]
* [imaging](https://github.com/disintegration/imaging) **star:2539** 简单的Go图像处理包。   ![star > 1000][Silver]
* [gocv](https://github.com/hybridgroup/gocv) **star:2478** 使用OpenCV 3.3+实现的计算机视觉(ComputerVision)的Go语言包。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [ln](https://github.com/fogleman/ln) **star:2465** Go实现的3D线艺术（3D Line Art）渲染。   ![star > 1000][Silver]
* [resize](https://github.com/nfnt/resize) **star:2133** Go实现的使用常用的插值法（interpolation methods）调整图像大小的库。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [gg](https://github.com/fogleman/gg) **star:1920** 纯Go语言实现的2D渲染。   ![star > 1000][Silver]
* [pt](https://github.com/fogleman/pt) **star:1775** Go实现的路径跟踪（path tracing）引擎。   ![star > 1000][Silver]
* [svgo](https://github.com/ajstarks/svgo) **star:1336**  Go实现的SVG生成库。   ![star > 1000][Silver]
* [smartcrop](https://github.com/muesli/smartcrop) **star:1262** 为任意图片寻找合适的位置进行图片裁剪。   ![star > 1000][Silver]
* [gift](https://github.com/disintegration/gift) **star:1214** 图像处理包。   ![star > 1000][Silver]
...
* [geopattern](https://github.com/pravj/geopattern) **star:1012** 由字符串创建漂亮图案的图片生成器。   ![star > 1000][Silver]
* [imagick](https://github.com/gographics/imagick) **star:983**  ImageMagick下MagickWand的C API的Go binding。   ![star > 100][Bronze]
* [bimg](https://github.com/h2non/bimg) **star:803** 使用libvips实现的快速高效的图像处理包。   ![star > 100][Bronze]
* [stegify](https://github.com/DimitarPetrov/stegify) **star:509**  Go实现的LSB隐写（LSB steganography），能够隐藏任何文件到一个图像中。   ![star > 100][Bronze]
* [mort](https://github.com/aldor007/mort) **star:365** Go语言实现的图像存储和处理服务器。   ![star > 100][Bronze]
* [govatar](https://github.com/o1egl/govatar) **star:309** 生成有趣头像的库和CMD工具。   ![star > 100][Bronze]
* [image2ascii](https://github.com/qeesung/image2ascii) **star:294** 将图像转换为ASCII码。   ![star > 100][Bronze]
...
* [darkroom](https://github.com/gojek/darkroom) **star:65** An image proxy with changeable storage backends and image processing engines with focus on speed and resiliency.   ![最近一周有更新][Green]
* [go-gd](https://github.com/bolknote/go-gd) **star:50**  GD库的Go binding。   ![最近一年没有更新][Yellow]
* [gltf](https://github.com/qmuntal/gltf) **star:38** 一个高效、健壮的glTF 2.0文件读取、写入和验证器。   ![最近一周有更新][Green]
...
* [flogo](https://github.com/tibcosoftware/flogo) **star:1138** Flogo是一个面向物联网边缘应用和集成的开源框架。   ![star > 1000][Silver]
...
* [mainflux](https://github.com/Mainflux/mainflux) **star:597** 工业物联网消息和设备管理服务器。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [gron](https://github.com/roylee0704/gron) **star:637** 使用简单的Go API定义基于时间的任务。 之后Gron的调度程序将运行它们。   ![star > 100][Bronze]
* [JobRunner](https://github.com/bamzi/jobrunner) **star:576** 智能和功能丰富的cron作业调度程序（包含任务队列和实时监控）。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [clockwork](https://github.com/whiteShtef/clockwork) **star:78** Go实现的简单直观的作业调度库。
...
* [GJSON](https://github.com/tidwall/gjson) **star:4944** 使用一行代码获取JSON的值。   ![star > 1000][Silver]
* [gojson](https://github.com/ChimeraCoder/gojson) **star:2034** 从JSON自动生成Go的结构（struct）定义。   ![star > 1000][Silver]
...
* [logrus](https://github.com/Sirupsen/logrus) **star:12048** Go的结构化日志操作 。   ![star > 5000][Gold]
* [zap](https://github.com/uber-go/zap) **star:7518** 快速、结构化、多等级的日志记录。   ![star > 5000][Gold]
* [spew](https://github.com/davecgh/go-spew) **star:3324** 为Go数据结构实现一个漂亮的printer用于帮助调试。   ![star > 1000][Silver]
* [glog](https://github.com/golang/glog) **star:2312** 为Go提供了多等级日志记录。   ![star > 1000][Silver]
* [zerolog](https://github.com/rs/zerolog) **star:2253** Zero-allocation JSON日志记录。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [tail](https://github.com/hpcloud/tail) **star:1546** 努力模拟实现BSD的tail的特性的Go包。   ![star > 1000][Silver]
* [lumberjack](https://github.com/natefinch/lumberjack) **star:1460** 简单的滚动日志，io.WriteCloser的实现。   ![star > 1000][Silver]
* [seelog](https://github.com/cihub/seelog) **star:1361** 具有灵活调度、过滤和格式化的日志功能。   ![star > 1000][Silver]
* [log15](https://github.com/inconshreveable/log15) **star:915** 简单、强大的日志操作。   ![star > 100][Bronze]
...
* [logutils](https://github.com/hashicorp/logutils) **star:249** Go的用于更好地进行日志操作的实用程序，继承了标准日志库。   ![star > 100][Bronze]
...
* [GoLearn](https://github.com/sjwhitworth/golearn) **star:6663** 通用机器学习库。   ![star > 5000][Gold]   ![包含中文文档][CN]
* [gorgonia](https://github.com/chewxy/gorgonia) **star:2707** 基于图形（graph-based）的计算库，如Theano：它为构建各种机器学习和神经网络算法提供了基本框架。   ![star > 1000][Silver]
...
* [goml](https://github.com/cdipaolo/goml) **star:1014** 在线机器学习。   ![star > 1000][Silver]
* [gosseract](https://github.com/otiai10/gosseract) **star:885** 使用c++的Tesseract库实现的OCR。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [CloudForest](https://github.com/ryanbressler/CloudForest) **star:645** 快速、灵活、多线程集成的决策树，用于机器学习。   ![star > 100][Bronze]
* [bayesian](https://github.com/jbrukh/bayesian) **star:631** Go的朴素贝叶斯分类。   ![star > 100][Bronze]
* [eaopt](https://github.com/MaxHalford/eaopt) **star:624** 一个进化优化（evolutionary optimization）库。   ![star > 100][Bronze]
* [gorse](https://github.com/zhenghaoz/gorse) **star:540** 基于协同过滤（Collaborative Filtering ）的高性能推荐系统包。   ![star > 100][Bronze]   ![包含中文文档][CN]
* [gobrain](https://github.com/goml/gobrain) **star:388** 用 Go 编写的神经网络库。   ![star > 100][Bronze]
* [regommend](https://github.com/muesli/regommend) **star:249** 推荐和协同过滤引擎。   ![star > 100][Bronze]
* [ocrserver](https://github.com/otiai10/ocrserver) **star:226** 一个简单的OCR API服务器，非常容易地使用Docker和Heroku部署。   ![star > 100][Bronze]
* [go-deep](https://github.com/patrikeh/go-deep) **star:221** 一个功能丰富的神经网络库 。   ![star > 100][Bronze]
* [go-galib](https://github.com/thoj/go-galib) **star:172** 用Go编写的遗传算法库。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [onnx-go](https://github.com/owulveryck/onnx-go) **star:152** Go Interface， 用于开放式神经网络交换(Open Neural Network Exchange)。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [Goptuna](https://github.com/c-bata/goptuna) **star:81** Bayesian optimization framework for black-box functions written in Go. Everything will be optimized.   ![最近一周有更新][Green]
...
* [fonet](https://github.com/Fontinalis/fonet) **star:33** 一个用Go编写的深度神经网络库。
...
* [sarama](https://github.com/Shopify/sarama) **star:4671**  Apache Kafka的Go库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gorush](https://github.com/appleboy/gorush) **star:3733** 使用[APNs2](https://github.com/sideshow/apns2)和谷歌[GCM](https://github.com/google/go-gcm)推送通知服务器。   ![star > 1000][Silver]
* [Centrifugo](https://github.com/centrifugal/centrifugo) **star:3703** 实时消息(Websockets或SockJS)服务器。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [machinery](https://github.com/RichardKnop/machinery) **star:3377** 基于分布式消息传递的异步任务/作业队列。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-socket.io](https://github.com/googollee/go-socket.io) **star:2896** go的socket.io库，一个实时应用程序框架。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [NATS Go Client](https://github.com/nats-io/nats) **star:2410** 轻量级和高性能的发布-订阅(publish-subscribe)和分布式队列消息传递系统——这是一个Go库。   ![star > 1000][Silver]
* [APNs2](https://github.com/sideshow/apns2) **star:2047** HTTP / 2苹果消息推送provider——发送推送消息到iOS, tvOS, Safari和OSX应用。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [Benthos](https://github.com/Jeffail/benthos) **star:2017** 一系列协议之间的消息流桥接。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gopush-cluster](https://github.com/Terry-Mao/gopush-cluster) **star:1840** gopush-cluster是一个gopush服务器集群。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]   ![包含中文文档][CN]
* [melody](https://github.com/olahol/melody) **star:1572** 处理websocket session的极简框架，包括广播和自动ping/pong处理。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [mangos](https://github.com/go-mangos/mangos) **star:1535** Nanomsg(“可伸缩协议”)的纯go实现,具有传输互操作性。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [Mercure](https://github.com/dunglas/mercure) **star:1518** 使用Mercure协议分派服务器发送(server-sent)更新的服务器和库(构建在服务器发送事件之上)。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-nsq](https://github.com/nsqio/go-nsq) **star:1464** NSQ的官方Go包。   ![star > 1000][Silver]
* [Uniqush-Push](https://github.com/uniqush/uniqush-push) **star:1099** Redis支持的统一推送服务, 用于服务端向移动设备的消息通知。   ![star > 1000][Silver]
...
* [Gollum](https://github.com/trivago/gollum) **star:769** 一个n:m多路复用器(n:m multiplexer)，收集不同来源的消息并将其广播到一组目的地。   ![star > 100][Bronze]
* [Beaver](https://github.com/Clivern/Beaver) **star:728** 一个实时消息服务器，可用于在web和手机app端构建一个可伸缩的应用内通知，多人游戏，聊天应用。   ![star > 100][Bronze]
* [EventBus](https://github.com/asaskevich/EventBus) **star:565** 具有异步兼容性的轻量级事件总线。   ![star > 100][Bronze]
...
* [emitter](https://github.com/olebedev/emitter) **star:311** 使用Go的方式发出事件, 带有通配符、谓词、取消可能性和许多其他优点。   ![star > 100][Bronze]
* [pubsub](https://github.com/tuxychandru/pubsub) **star:281** 简单的pubsub的go包。   ![star > 100][Bronze]
...
* [Bus](https://github.com/mustafaturan/bus) **star:114** 内部通信的最小消息总线实现。   ![star > 100][Bronze]
...
* [messagebus](https://github.com/vardius/message-bus) **star:69** messagebus是一个Go的简单异步消息总线，非常适合在执行事件sourcing、CQRS和DDD时用作事件总线。
...
* [Commander](https://github.com/jeroenrinzema/commander) **star:23** 高级事件驱动的消费者/生产者(consumer/producer)，支持各种“方言”，如Apache Kafka。   ![最近一周有更新][Green]
...
* [unioffice](https://github.com/unidoc/unioffice) **star:1740** 用于创建和处理Office Word (.docx)、Excel (.xlsx)和Powerpoint (.pptx)文档的纯go库。   ![star > 1000][Silver]

### Microsoft Excel

*用于操作Microsoft Excel的库。 (翻译出错了? 试试 [英文版](README_EN.md#microsoft-excel) 吧~)*

* [excelize](https://github.com/360EntSecGroup-Skylar/excelize) **star:4509** 用于读写Microsoft Excel™(XLSX)文件的Go语言库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [xlsx](https://github.com/tealeg/xlsx) **star:3446** 用以简化在Go程序中读取使用最新版本Microsoft Excel的XML格式文件的库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [xlsx](https://github.com/plandem/xlsx) **star:75** 在Go程序以快速和安全的方式读取/更新现有的Microsoft Excel文件。   ![最近一周有更新][Green]
...
* [dig](https://github.com/uber-go/dig) **star:927** 一个基于反射的Go依赖注入工具包。   ![star > 100][Bronze]
* [fx](https://github.com/uber-go/fx) **star:769** 基于依赖注入的Go应用程序框架(构建在dig之上)。   ![star > 100][Bronze]
...
* [golang-standards/project-layout](https://github.com/golang-standards/project-layout) **star:9322** Go生态系统中历史和新兴的项目布局模式集合。   ![star > 5000][Gold]
...
* [gopsutil](https://github.com/shirou/gopsutil) **star:3952** 用于检索进程和系统利用率(CPU、内存、磁盘等)的跨平台的库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [archiver](https://github.com/mholt/archiver) **star:2498** 用于生成和解压.zip和.tar.gz文档的库和命令。   ![star > 1000][Silver]   ![最近一周有更新][Green]
...
* [go-commons-pool](https://github.com/jolestar/go-commons-pool) **star:676** Go语言的通用对象池。   ![star > 100][Bronze]   ![包含中文文档][CN]
...
* [shortid](https://github.com/teris-io/shortid) **star:452** 分布式地生成超短、唯一、非顺序、URL友好的id。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [gofakeit](https://github.com/brianvoe/gofakeit) **star:431** 用go编写的随机数据生成器。   ![star > 100][Bronze]
* [llvm](https://github.com/llir/llvm) **star:415** 用于在纯Go中与LLVM IR交互的库。   ![star > 100][Bronze]
...
* [banner](https://github.com/dimiro1/banner) **star:236** 在Go应用程序中添加漂亮的横幅(banner)。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [gountries](https://github.com/pariz/gountries) **star:211** 获取国家和细节数据的包。   ![star > 100][Bronze]
* [antch](https://github.com/antchfx/antch) **star:143** 一个快速、强大和可扩展的web爬虫框架。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]   ![包含中文文档][CN]
...
* [gommit](https://github.com/antham/gommit) **star:75** 分析git提交消息，确保它们遵循已定义的格式。   ![最近一周有更新][Green]
...
* [xkg](https://github.com/go-xkg/xkg) **star:41** X Keyboard Grabber(键盘事件捕获)   ![最近一年没有更新][Yellow]
* [pdfgen](https://github.com/hyperboloide/pdfgen) **star:34** 通过Json请求生成PDF的HTTP服务。   ![最近一年没有更新][Yellow]
* [ghorg](https://github.com/gabrie30/ghorg) **star:32** GitHub一个组织中所有的仓库复制到一个目录中。   ![最近一周有更新][Green]
...
* [prose](https://github.com/jdkato/prose) **star:2087** 用于支持标记化、词性标记、名称实体提取等文本处理的库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gse](https://github.com/go-ego/gse) **star:1080** 高效的文本分割;支持英语、汉语、日语等。   ![star > 1000][Silver]   ![包含中文文档][CN]
* [when](https://github.com/olebedev/when) **star:927** 带有可插入规则的自然EN和RU语言日期/时间解析器。   ![star > 100][Bronze]
* [gojieba](https://github.com/yanyiwu/gojieba) **star:827** 这是一个Go实现的[jieba](https://github.com/fxsjy/jieba)，这是一个中文分词算法。   ![star > 100][Bronze]   ![包含中文文档][CN]
* [go-pinyin](https://github.com/mozillazg/go-pinyin) **star:531** 中文汉字到汉语拼音的转换。   ![star > 100][Bronze]
* [kagome](https://github.com/ikawaha/kagome) **star:417** JP形态学分析仪编写的纯Go。   ![star > 100][Bronze]
* [whatlanggo](https://github.com/abadojack/whatlanggo) **star:354** Go的自然语言检测包。支持84种语言和24种脚本(书写系统，如拉丁语、西里尔语等)。   ![star > 100][Bronze]
...
* [go2vec](https://github.com/danieldk/go2vec) **star:31** 用于word2vec嵌入式的阅读器和实用程序函数。
...
* [kcptun](https://github.com/xtaci/kcptun) **star:10707** 基于KCP协议的非常简单和快速udp隧道。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [fasthttp](https://github.com/valyala/fasthttp) **star:9440** 一个快速HTTP实现，比net/http快10倍。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [dns](https://github.com/miekg/dns) **star:3837** 用于 DNS 的库。   ![star > 1000][Silver]
* [HTTPLab](https://github.com/gchaincl/httplab) **star:3414** HTTPLabs 允许你检查 HTTP 请求和伪造响应。   ![star > 1000][Silver]
* [quic-go](https://github.com/lucas-clemente/quic-go) **star:2954** 在纯Go中实现了QUIC协议。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gopacket](https://github.com/google/gopacket) **star:2902** Go library for packet processing with libpcap bindings.   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [webrtc](https://github.com/pions/webrtc) **star:2310** WebRTC API的纯Go实现。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [kcp-go](https://github.com/xtaci/kcp-go) **star:2254** 快速可靠的ARQ协议。   ![star > 1000][Silver]
* [gobgp](https://github.com/osrg/gobgp) **star:1697** 基于 Go 的 BGP 实现。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [ssh](https://github.com/gliderlabs/ssh) **star:1117** 用于构建SSH服务器的高级API(封装密码/ SSH)。   ![star > 1000][Silver]
* [fortio](https://github.com/fortio/fortio) **star:895** 负载测试库和命令行工具，高级的echo服务器和web UI。允许指定一组每秒查询的负载，并记录延迟直方图和其他有用的统计数据，并将它们作图。支持Tcp、Http、gRPC。   ![star > 100][Bronze]
* [water](https://github.com/songgao/water) **star:849** 简单TUN / TAP图书馆。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [go-getter](https://github.com/hashicorp/go-getter) **star:730**  通过URL来下载文件或目录。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [NFF-Go](https://github.com/intel-go/nff-go) **star:667** 用于快速开发云计算和裸机网络功能的框架(原YANFF)。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [mqttPaho](https://eclipse.org/paho/clients/golang/)  Paho Go客户端提供了一个 MQTT 客户端库，用于通过TCP、TLS或WebSockets连接到MQTT代理。
* [mdns](https://github.com/hashicorp/mdns) **star:554** 简单mDNS(Multicast DNS)客户端/服务器库。   ![star > 100][Bronze]
* [grab](https://github.com/cavaliercoder/grab) **star:548**  用于管理文件下载。   ![star > 100][Bronze]
* [ftp](https://github.com/jlaffaye/ftp) **star:533** 实现了[RFC 959](http://tools.ietf.org/html/rfc959)中描述的ftp客户端。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [lhttp](https://github.com/fanux/lhttp) **star:514** 强大的websocket框架，可以让你更容易的构建IM服务器。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]   ![包含中文文档][CN]
* [gosnmp](https://github.com/soniah/gosnmp) **star:440** 用于执行 SNMP 操作的原生 Go 库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [gotcp](https://github.com/gansidui/gotcp) **star:419** 用于快速编写 tcp 应用程序。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [cidranger](https://github.com/yl2chen/cidranger) **star:388** 快速得 IP 到 CIDR 查找。   ![star > 100][Bronze]
* [peerdiscovery](https://github.com/schollz/peerdiscovery) **star:366** 基于UDP组播的跨平台本地对等点发现库。   ![star > 100][Bronze]
...
* [stun](https://github.com/go-rtc/stun) **star:290** Go实现的RFC 5389 STUN协议。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [tcp_server](https://github.com/firstrow/tcp_server) **star:286**  Go 图书馆建设tcp服务器更快。   ![star > 100][Bronze]
...
* [gNxI](https://github.com/google/gnxi) **star:105** 一组基于 gNMI 和 gNOI 协议的网络管理工具。   ![star > 100][Bronze]
...
* [grequests](https://github.com/levigross/grequests) **star:1419** 一个 Go “克隆”的伟大和著名的请求库。   ![star > 1000][Silver]
* [heimdall](https://github.com/gojektech/heimdall) **star:1088** 具有重试和hystrix功能的增强http客户机。   ![star > 1000][Silver]
* [sling](https://github.com/dghubble/sling) **star:1013** Sling是一个用于创建和发送API请求的Go HTTP客户端库。   ![star > 1000][Silver]
...
* [rq](https://github.com/ddo/rq) **star:29** golang stdlib HTTP客户端更好的接口。

## OpenGL

*用于在Go中使用OpenGL的库。 (翻译出错了? 试试 [英文版](README_EN.md#opengl) 吧~)*

* [glfw](https://github.com/go-gl/glfw) **star:737** GLFW 3 的 Go 接口实现。   ![star > 100][Bronze]
* [gl](https://github.com/go-gl/gl) **star:645** OpenGL 的 Go 接口实现(通过glow生成)。   ![star > 100][Bronze]
* [mathgl](https://github.com/go-gl/mathgl) **star:292** 完全基于 Go 实现的数学软件包，专门用于处理三维数学。借鉴于 GLM。   ![star > 100][Bronze]
...
* [GORM](https://github.com/jinzhu/gorm) **star:14816** 一个出色的 ORM 库。主要目标是对开发人员友好。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Xorm](https://github.com/go-xorm/xorm) **star:5245** 基于 Go 的简单而强大的ORM。   ![star > 5000][Gold]   ![最近一周有更新][Green]   ![包含中文文档][CN]
* [gorp](https://github.com/go-gorp/gorp) **star:3083** 基于 Go 的关系持久性 ORM-ish 库。   ![star > 1000][Silver]
* [go-pg](https://github.com/go-pg/pg) **star:3032** 用于 PostgreSQL 的ORM。侧重于 PostgreSQL 的特性和性能。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [SQLBoiler](https://github.com/volatiletech/sqlboiler) **star:2281** ORM 生成器。根据数据库 schema 生成一个功能强大且运行速度快的ORM。   ![star > 1000][Silver]
* [upper.io/db](https://github.com/upper/db) **star:1863** 对外提供统一的接口用于访问不同的存储介质，例如PostgreSQL, MySQL, SQLite, MSSQL, QL、MongoDB.。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [reform](https://github.com/go-reform/reform) **star:806** 基于非空接口和代码生成的 ORM。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [pop/soda](https://github.com/gobuffalo/pop) **star:686** 数据库迁移、创建、ORM等。用于MySQL、PostgreSQL和SQLite。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [QBS](https://github.com/coocood/qbs) **star:541** Stands for Query By Struct. A Go ORM.   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]   ![包含中文文档][CN]
* [go-queryset](https://github.com/jirfag/go-queryset) **star:452** 基于 GORM 100% 类型安全的 ORM。可支持 MySQL, PostgreSQL, Sqlite3, SQL Server。   ![star > 100][Bronze]
* [beego orm](https://github.com/astaxie/beego/tree/master/orm)  强大的orm框架。支持: pq/mysql/sqlite3。
* [Zoom](https://github.com/albrow/zoom) **star:241** 基于 Redis 的快速数据存储和查询引擎。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [go-sqlbuilder](https://github.com/huandu/go-sqlbuilder) **star:239** 一个灵活而强大的SQL字符串构建器库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [go-store](https://github.com/gosuri/go-store) **star:94** 简单且快速的 Redis 键值存储库。   ![最近一年没有更新][Yellow]
...
* [go-firestorm](https://github.com/jschoedt/go-firestorm) **star:2** 一个轻量级的ORM。用于Google/Firebase Cloud Firestore。
...
* [dep](https://github.com/golang/dep) **star:12605**  Go 的依赖管理工具，需要 Go 1.9+   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [vgo](https://go.googlesource.com/vgo/)  Go 命令版本管理

*用于包和依赖项管理的非官方库。 (翻译出错了? 试试 [英文版](README_EN.md#package-management) 吧~)*

* [glide](https://github.com/Masterminds/glide) **star:7791** 轻松管理您的 golang 第三方包。受Maven、Bundler和Pip等工具的启发。   ![star > 5000][Gold]
...
* [govendor](https://github.com/kardianos/govendor) **star:4758** 包管理器。使用 vendor 文件的 Go vendor 工具。   ![star > 1000][Silver]
* [gopm](https://github.com/gpmgo/gopm) **star:2371** 包管理器。   ![star > 1000][Silver]   ![归档项目][Archived]
...
* [jaeger](https://github.com/jaegertracing/jaeger) **star:8738** 分布式跟踪系统。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [profile](https://github.com/pkg/profile) **star:1014** Go的简单分析支持包。   ![star > 1000][Silver]
* [tracer](https://github.com/kamilsk/tracer) **star:9** 简单、轻量级的跟踪。

## 查询语言

* [graphql-go](https://github.com/graphql-go/graphql) **star:5271** 为Go实现GraphQL。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [graphql](https://github.com/neelance/graphql-go) **star:2810** 关注易用性的GraphQL服务器。   ![star > 1000][Silver]
* [gojsonq](https://github.com/thedevsaddam/gojsonq) **star:866** 一个用来查询JSON数据的Go包。   ![star > 100][Bronze]
* [jsonql](https://github.com/elgs/jsonql) **star:201** Golang中的JSON查询表达式库。   ![star > 100][Bronze]
* [rql](https://github.com/a8m/rql) **star:111** 用于REST API的资源查询语言。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [packr](https://github.com/gobuffalo/packr) **star:2154** 将静态文件嵌入到Go二进制文件中的简单方法。   ![star > 1000][Silver]
* [statik](https://github.com/rakyll/statik) **star:2132** 将静态文件嵌入到Go可执行文件中。   ![star > 1000][Silver]
* [go.rice](https://github.com/GeertJohan/go.rice) **star:1666** go.rice 是一个Go包，它使处理html、js、css、图像和模板等资源变得非常容易。   ![star > 1000][Silver]
* [vfsgen](https://github.com/shurcooL/vfsgen) **star:663** 生成一个vfsdata。静态实现给定虚拟文件系统的go文件。   ![star > 100][Bronze]
* [esc](https://github.com/mjibson/esc) **star:474** 将文件嵌入到Go程序中并提供http文件系统接口。   ![star > 100][Bronze]
...
* [go-embed](https://github.com/pyros2097/go-embed) **star:17** 生成go代码，将资源文件嵌入到库或可执行文件中。

## 科学与数据分析

*用于科学计算和数据分析的库。 (翻译出错了? 试试 [英文版](README_EN.md#science-and-data-analysis) 吧~)*

* [gonum](https://github.com/gonum/gonum) **star:2985** Gonum是一组用于Go编程语言的数字库。它包含用于矩阵、统计、优化等的库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [stats](https://github.com/montanaflynn/stats) **star:1347** 包含Golang标准库中缺少的公共函数的统计软件包。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [streamtools](https://github.com/nytlabs/streamtools) **star:1314** 通用图形工具，用于处理数据流。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [gosl](https://github.com/cpmech/gosl) **star:1309** 提供线性代数，FFT，几何，NURBS，数值方法，概率，优化，微分方程，等等。   ![star > 1000][Silver]
* [gonum/plot](https://github.com/gonum/plot) **star:1222** gonum/plot提供了一个API，用于在Go中构建和绘制绘图。   ![star > 1000][Silver]
* [go-dsp](https://github.com/mjibson/go-dsp) **star:628** Go数字信号处理。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [goraph](https://github.com/gyuho/goraph) **star:599** 纯Go图论库(数据结构，算法可视化)。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [chart](https://github.com/vdobler/chart) **star:582** 简单的图表绘制库。支持多种图形类型。   ![star > 100][Bronze]
...
* [orb](https://github.com/paulmach/orb) **star:200** 2D几何类型，支持剪切、GeoJSON和Mapbox矢量平铺。   ![star > 100][Bronze]
* [gohistogram](https://github.com/VividCortex/gohistogram) **star:127** 数据流的近似直方图。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [dataframe-go](https://github.com/rocketlaunchr/dataframe-go) **star:79** 用于机器学习和统计的数据模型(类似于 pandas)。
* [sparse](https://github.com/james-bowman/sparse) **star:70**  Go 稀疏矩阵格式的线性代数支持科学和机器学习应用程序，兼容gonum矩阵库。
...
* [lego](https://github.com/xenolf/lego) **star:3541** 纯 Go ACME 客户端库及命令行工具   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [Cameradar](https://github.com/Ullaakut/cameradar) **star:1831** 工具和库，以远程入侵RTSP流从监控摄像头。   ![star > 1000][Silver]
* [acmetool](https://github.com/hlandau/acme) **star:1696** ACME(让我们用自动更新加密)客户端工具。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [memguard](https://github.com/awnumar/memguard) **star:1535** 一个用于处理内存中敏感值的纯Go库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [secure](https://github.com/unrolled/secure) **star:1215** Go 语言 HTTP 中间件，为 Go 提供了一些安全功能   ![star > 1000][Silver]
* [acra](https://github.com/cossacklabs/acra) **star:454** 网络加密代理保护基于数据库的应用程序免受数据泄漏:强选择性加密，SQL注入预防，入侵检测系统。   ![star > 100][Bronze]
* [nacl](https://github.com/kevinburke/nacl) **star:452**  Go 实现NaCL API的集合。   ![star > 100][Bronze]
...
* [go-yara](https://github.com/hillu/go-yara) **star:134** YARA的 Go 语言接口，号称是 “对于恶意软件研究者（以及其他人）来说是模式匹配的瑞士军刀”   ![star > 100][Bronze]
...
* [jsoniter](https://github.com/json-iterator/go) **star:5590** 高性能，100% 兼容的“encoding/json” 替代品   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [goprotobuf](https://github.com/golang/protobuf) **star:5136** 通过库和协议编译器插件使 Go 语言支持 Google的 protocol buffers.   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [gogoprotobuf](https://github.com/gogo/protobuf) **star:2989** Go 语言的 Protocol Buffer 库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [mapstructure](https://github.com/mitchellh/mapstructure) **star:2497** 用于对原生键值对进行解码生成 Go 语言结构体   ![star > 1000][Silver]
* [go-codec](https://github.com/ugorji/go) **star:1240** 高性能、多功能、规范化编码解码以及 rpc 库， 用于 msgpack, cbor 和 json，支持基于运行时的 OR 码生成   ![star > 1000][Silver]   ![最近一周有更新][Green]
...
* [php_session_decoder](https://github.com/yvasiyarov/php_session_decoder) **star:120** 用于协同 PHP session 格式数据和 PHP 序列化／反序列化函数工作的go语言库   ![star > 100][Bronze]
...
* [etcd](https://github.com/coreos/etcd) **star:26673** 为共享配置和服务发现提供高可用的键值存储。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Caddy](https://github.com/mholt/caddy) **star:23268** Caddy是另一种HTTP/2 web服务器，易于配置和使用。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [consul](https://www.consul.io/)  Consul 是一个用于服务发现、监控和配置的工具
* [minio](https://github.com/minio/minio) **star:17608** Minio是一个分布式对象存储服务器。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [RoadRunner](https://github.com/spiral/roadrunner) **star:3324** 高性能PHP应用服务器，负载平衡器和进程管理器。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [devd](https://github.com/cortesi/devd) **star:2812** 为开发人员提供本地web服务器。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [algernon](https://github.com/xyproto/algernon) **star:1591** 内置支持Lua、Markdown、GCSS和Amber的HTTP/2 web服务器。   ![star > 1000][Silver]
* [flipt](https://github.com/markphelps/flipt) **star:992** 一个用Go和Vue.js编写的自包含特性标志解决方案   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [SFTPGo](https://github.com/drakkan/sftpgo) **star:907** 功能齐全，高度可配置的SFTP服务器软件。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [yakvs](https://git.sci4me.com/sci4me/yakvs)  小型化、网络化、基于内存的键值存储
* [Flagr](https://github.com/checkr/flagr) **star:831** Flagr是一个开源特性标记和A/B测试服务。   ![star > 100][Bronze]
* [Fider](https://github.com/getfider/fider) **star:802** Fider是一个收集和组织客户反馈的开放平台。   ![star > 100][Bronze]
* [jackal](https://github.com/ortuman/jackal) **star:719** 用Go编写的XMPP服务器。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [discovery](https://github.com/Bilibili/discovery) **star:680** 用于弹性中间层负载平衡和故障转移的注册表。   ![star > 100][Bronze]
...
* [go-streams](https://github.com/reugn/go-streams) **star:190** 流处理库。   ![star > 100][Bronze]

## 模板引擎

*用于模板和词法分析的库和工具。 (翻译出错了? 试试 [英文版](README_EN.md#template-engines) 吧~)*

* [gofpdf](https://github.com/jung-kurt/gofpdf) **star:3101** PDF 文档生成器，支持文本，绘图和图片   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [pongo2](https://github.com/flosch/pongo2) **star:1494** 类似 DjanGo 的模板引擎   ![star > 1000][Silver]
* [quicktemplate](https://github.com/valyala/quicktemplate) **star:1415** 快速、强大且易用的模板引擎。将模板转化为 Go 语言并进行编译   ![star > 1000][Silver]
* [hero](https://github.com/shiyanhui/hero) **star:1207** Hero是一个方便、快速和强大的go模板引擎。   ![star > 1000][Silver]   ![包含中文文档][CN]
...
* [fasttemplate](https://github.com/valyala/fasttemplate) **star:302** 简单快速的模板引擎。进行模板元素替换时，速度是比[text/template](http://golang.org/pkg/text/template/)快10倍。   ![star > 100][Bronze]
...
* Testing Frameworks
    ...
    * [Testify](https://github.com/stretchr/testify) **star:8230** 对标准测试包的扩展。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [go-cmp](https://github.com/google/go-cmp) **star:1179** 用于比较测试中的Go值的包。   ![star > 1000][Silver]
    * [httpexpect](https://github.com/gavv/httpexpect) **star:1146** 简洁的、声明式的、易用的端到端HTTP 及 REST API 测试   ![star > 1000][Silver]
    * [godog](https://github.com/DATA-DOG/godog) **star:767** 类似 Cucumber 或 Behat 的 BDD 框架   ![star > 100][Bronze]
    ...
    * [go-mutesting](https://github.com/zimmski/go-mutesting) **star:296** 变异测试的Go源代码。   ![star > 100][Bronze]
    ...
    * [frisby](https://github.com/verdverm/frisby) **star:250** REST API测试框架。   ![star > 100][Bronze]
    ...
    * [gotest.tools](https://github.com/gotestyourself/gotest.tools) **star:122** 一组包，用于增强go测试包并支持公共模式。   ![star > 100][Bronze]
    * [GoSpec](https://github.com/orfjackal/gospec) **star:111** 用于 Go 编程语言的bdd风格的测试框架。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
    * [endly](https://github.com/viant/endly) **star:97** 声明性端到端功能测试。
    * [dbcleaner](https://github.com/khaiql/dbcleaner) **star:88** 清空数据库用于测试，受到database_cleaner 的启发
...
* Mock
    * [gomock](https://github.com/golang/mock) **star:2886** 用于Go编程语言的mock框架。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [go-sqlmock](https://github.com/DATA-DOG/go-sqlmock) **star:1761** Mock SQL ，用于测试数据库交互   ![star > 1000][Silver]
    * [hoverfly](https://github.com/SpectoLabs/hoverfly) **star:1446** 使用可扩展中间件和易于使用的CLI记录和模拟REST/SOAP api的HTTP(S)代理。   ![star > 1000][Silver]
    * [gock](https://github.com/h2non/gock) **star:826** 多功能、易用 HTTP mock   ![star > 100][Bronze]
    * [httpmock](https://github.com/jarcoal/httpmock) **star:588** 轻松模拟来自外部资源的HTTP响应。   ![star > 100][Bronze]
    * [counterfeiter](https://github.com/maxbrunsfeld/counterfeiter) **star:364** 用于生成自包含 mock 对象的工具   ![star > 100][Bronze]
    * [minimock](https://github.com/gojuno/minimock) **star:268** Go接口的模拟生成器。   ![star > 100][Bronze]
    * [go-txdb](https://github.com/DATA-DOG/go-txdb) **star:165** 基于单事务的数据库驱动，主要用于测试目的   ![star > 100][Bronze]
...
* Fuzzing and delta-debugging/reducing/shrinking.
    * [go-fuzz](https://github.com/dvyukov/go-fuzz) **star:2899** 随机测试系统。   ![star > 1000][Silver]
    * [gofuzz](https://github.com/google/gofuzz) **star:536** 用于生成随机值来初始化 Go 语言对象的库   ![star > 100][Bronze]
    * [Tavor](https://github.com/zimmski/tavor) **star:212** 通用模糊测试框架   ![star > 100][Bronze]

* Selenium and browser control tools.
    * [chromedp](https://github.com/knq/chromedp) **star:3622** 用于驱动和测试 Chrome, Safari, Edge, Android Webviews, 以及其他支持 Chrome 调试协议的产品   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [selenoid](https://github.com/aerokube/selenoid) **star:1245** Selenium hub 服务器的替代品，在容器中启动浏览器   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [cdp](https://github.com/mafredri/cdp) **star:355** 用于Chrome调试协议的类型安全绑定，可与实现该协议的浏览器或其他调试目标一起使用。   ![star > 100][Bronze]
    * [ggr](https://github.com/aerokube/ggr) **star:210** 一个轻量级服务器，可以将 Selenium Wedriver 的请求路由或代理到多个 Selenium hubs   ![star > 100][Bronze]

* Fail injection
    * [failpoint](https://github.com/pingcap/failpoint) **star:396** 为Golang实现[failpoints](http://www.freebsd.org/cgi/man.cgi?query=fail)。   ![star > 100][Bronze]
...
* Specific Formats
    * [colly](https://github.com/asciimoo/colly) **star:8510** 快速和优雅的 Scraping 框架。   ![star > 5000][Gold]   ![最近一周有更新][Green]
    * [GoQuery](https://github.com/PuerkitoBio/goquery) **star:7633** GoQuery 为 Go 语言带来了一组类似 jQuery 的语法和功能   ![star > 5000][Gold]
    * [blackfriday](https://github.com/russross/blackfriday) **star:3919** Markdown 解析器   ![star > 1000][Silver]
    * [toml](https://github.com/BurntSushi/toml) **star:2808** TOML配置格式(带反射的编码器/解码器)。   ![star > 1000][Silver]
    * [sh](https://github.com/mvdan/sh) **star:2027** Shell解析器和格式化工具。   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [go-humanize](https://github.com/dustin/go-humanize) **star:1907** 格式化程序，用于将时间、数字和内存大小转换为可读格式。   ![star > 1000][Silver]
    * [bluemonday](https://github.com/microcosm-cc/bluemonday) **star:1259** HTML 清理工具   ![star > 1000][Silver]   ![最近一周有更新][Green]
    * [inject](https://github.com/facebookgo/inject) **star:1145** 包注入提供了一个基于反射的注入器。   ![star > 1000][Silver]   ![归档项目][Archived]
    * [gofeed](https://github.com/mmcdole/gofeed) **star:1104** 在Go中解析RSS和Atom feeds。   ![star > 1000][Silver]
    * [go-toml](https://github.com/pelletier/go-toml) **star:611** 使用带有查询支持和方便的cli工具的TOML格式库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
    * [commonregex](https://github.com/mingrammer/commonregex) **star:554** 一组用于Go的公共正则表达式。   ![star > 100][Bronze]
    * [slug](https://github.com/gosimple/slug) **star:383** URL 友好的 slug 化工具，支持多种语言   ![star > 100][Bronze]   ![最近一周有更新][Green]
    ...
    * [gotext](https://github.com/leonelquinteros/gotext) **star:232** GNU gettext 工具   ![star > 100][Bronze]
    * [go-runewidth](https://github.com/mattn/go-runewidth) **star:210** 函数获取字符或字符串的固定宽度。   ![star > 100][Bronze]
    * [goq](https://github.com/andrewstuart/goq) **star:148**  声明式 HTML 编组，使用结构标签和 jQuery 语法 (使用 GoQuery).   ![star > 100][Bronze]
    * [htmlquery](https://github.com/antchfx/htmlquery) **star:131** 用于HTML的XPath查询包，允许您通过XPath表达式从HTML文档中提取数据或求值。   ![star > 100][Bronze]
    * [go-nmea](https://github.com/adrianmo/go-nmea) **star:100** 用于Go语言的NMEA解析器库。   ![star > 100][Bronze]
    * [sdp](https://github.com/gortc/sdp) **star:72** SDP:会话描述协议[[RFC 4566](https://tools.ietf.org/html/rfc4566)]。   ![最近一周有更新][Green]
    ...
    * [go-slugify](https://github.com/mozillazg/go-slugify) **star:53** 生成漂亮的固定链接地址（slug），支持多种语言   ![最近一年没有更新][Yellow]
    ...
    * [gonameparts](https://github.com/polera/gonameparts) **star:29** 将人名解析为单独的名称部分。
    ...
    * [codetree](https://github.com/aerogo/codetree) **star:7** 解析缩进代码(python、pixy、scarlet等)并返回树结构。   ![最近一周有更新][Green]
...
* Utility
    ...
    * [gotabulate](https://github.com/bndr/gotabulate) **star:200** 使用 Go 语言简单、美观的打印表格数据   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
    * [radix](https://github.com/yourbasic/radix) **star:145** 快速字符串排序算法。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
    ...
    * [Tagify](https://github.com/zoomio/tagify) **star:2** 从给定源生成一组标记。

## 第三方api

*用于访问第三方api的库。 (翻译出错了? 试试 [英文版](README_EN.md#third-party-apis) 吧~)*

* [aws-sdk-go](https://github.com/aws/aws-sdk-go) **star:5063** AWS 提供的官方go语言 SDK   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [github](https://github.com/google/go-github) **star:4781** 访问GitHub REST API v3的库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [slack](https://github.com/nlopes/slack) **star:2434** Slack API。   ![star > 1000][Silver]
* [google](https://github.com/google/google-api-go-client) **star:1936** 为Go自动生成谷歌api。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [google-cloud](https://github.com/GoogleCloudPlatform/gcloud-golang) **star:1803** 谷歌云api Go 客户端库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [anaconda](https://github.com/ChimeraCoder/anaconda) **star:992**  Twitter 1.1 API 的 go 语言客户端   ![star > 100][Bronze]
* [discordgo](https://github.com/bwmarrin/discordgo) **star:973**  Discord Chat API的客户端。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [stripe](https://github.com/stripe/stripe-go) **star:947**  Stripe API 的 Go 语言客户端   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [facebook](https://github.com/huandu/facebook) **star:774** 支持 Facebook Graph API 的库   ![star > 100][Bronze]
* [minio-go](https://github.com/minio/minio-go) **star:725** 用于Amazon S3兼容云存储的Minio Go库。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [go-twitter](https://github.com/dghubble/go-twitter) **star:720**  Twitter v1.1 api 的客户端.   ![star > 100][Bronze]
* [go-jira](https://github.com/andygrunwald/go-jira) **star:582**  Go [Atlassian JIRA](https://www.atlassian.com/software/jira)的客户端库   ![star > 100][Bronze]
...
* [gads](https://github.com/emiddleton/gads) **star:43**  Google Adwords 非官方 API   ![最近一周有更新][Green]
...
* [fzf](https://github.com/junegunn/fzf) **star:23330** 用Go编写的命令行模糊查找器。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [hub](https://github.com/github/hub) **star:17103** 封装了 git 命令，提供了额外的功能用于在终端中和 Github 进行交互。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [delve](https://github.com/derekparker/delve) **star:12107** Go 语言调试器   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [ctop](https://github.com/bcicen/ctop) **star:8847** [Top-like](http://ctop.sh)接口(例如htop)， 用于容器数据收集。   ![star > 5000][Gold]
* [wuzz](https://github.com/asciimoo/wuzz) **star:8273** 用于HTTP检查的交互式cli工具。   ![star > 5000][Gold]
* [sqlx](https://github.com/jmoiron/sqlx) **star:6844** 为内建的数据库/sql 软件包提供一组扩展。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [peco](https://github.com/peco/peco) **star:5488** 简单的交互过滤工具。   ![star > 5000][Gold]
* [usql](https://github.com/knq/usql) **star:4680** usql 是一个通用的命令行接口，用于操作 sql 数据库。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [goreleaser](https://github.com/goreleaser/goreleaser) **star:4537** 尽可能快速的发布 Go 语言二进制文件。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [godropbox](https://github.com/dropbox/godropbox) **star:3745** 用于编写 Go 语言服务／应用的库，来自 Dropbox.。   ![star > 1000][Silver]
* [realize](https://github.com/tockins/realize) **star:3172** Go 语言构建系统，可以监控文件变化并重新加载。运行，构建，监控文件并支持自定义路径。   ![star > 1000][Silver]
* [goreporter](https://github.com/wgliang/goreporter) **star:2489** 进行代码静态分析，单元测试，代码检视并生成代码质量报告的工具   ![star > 1000][Silver]
* [panicparse](https://github.com/maruel/panicparse) **star:2144** 将类似的协程分组并对调用栈进行着色   ![star > 1000][Silver]
* [hystrix-go](https://github.com/afex/hystrix-go) **star:2036** 实现 Hystrix 风格的、程序员预定义的 fallback 机制（熔断。   ![star > 1000][Silver]
* [resty](https://github.com/go-resty/resty) **star:2008** 简单的 HTTP 和 REST 客户端，受到 Ruby rest-client 的启发。   ![star > 1000][Silver]
...
* [Storm](https://github.com/asdine/storm) **star:1364** 一个简单又强大的用于 BoltDB 的工具   ![star > 1000][Silver]
* [mole](https://github.com/davrodpin/mole) **star:1301** cli应用程序可以轻松创建ssh隧道。   ![star > 1000][Silver]
* [go-funk](https://github.com/thoas/go-funk) **star:1208** 现代 Go 语言工具库，提供了很多有用的工具 (map, find, contains, filter, chunk, reverse, ...)   ![star > 1000][Silver]
* [mc](https://github.com/minio/mc) **star:1124** Minio Client 提供了一组工具，用于操作 Amazon S3 兼容云存储和文件系统。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [filetype](https://github.com/h2non/filetype) **star:957** 通过数字签名来推测文件类型。   ![star > 100][Bronze]
* [boilr](https://github.com/tmrts/boilr) **star:946** 非常快的CLI工具，用于从样板模板创建项目。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [mergo](https://github.com/imdario/mergo) **star:857** 用于将结构体和map合并进 Go 语言的工具。对于配置默认值，避免杂乱的if语句很有帮助。   ![star > 100][Bronze]
* [spinner](https://github.com/briandowns/spinner) **star:802**  一个 Go 语言软件包，提供多种选项，方便在终端中创建加载动画。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [jump](https://github.com/gsamokovarov/jump) **star:665** 通过学习你的习惯，可以帮助你更快地导航。   ![star > 100][Bronze]
* [immortal](https://github.com/immortal/immortal) **star:608** \*nix 跨平台 (与操作系统无关的)监控程序。   ![star > 100][Bronze]
...
* [godaemon](https://github.com/VividCortex/godaemon) **star:406** 用于编写守护进程的工具   ![star > 100][Bronze]
* [request](https://github.com/mozillazg/request) **star:356** Go 语言版的 HTTP Requests for Humans™.。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
* [circuit](https://github.com/cep21/circuit) **star:330** 一个高效和功能齐全的 类似 Hystrix Go 实现断路器模式。   ![star > 100][Bronze]
* [ergo](https://github.com/cristianoliveira/ergo) **star:316** 管理运行在不同端口上的多个本地服务变得很容易。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [koazee](https://github.com/wesovilabs/koazee) **star:297** 库的灵感来自于延迟计算和函数式编程，从而减少了使用数组的麻烦。   ![star > 100][Bronze]
...
* [clockwork](https://github.com/jonboulle/clockwork) **star:223** 一个简单的假 clock 。   ![star > 100][Bronze]
* [Deepcopier](https://github.com/ulule/deepcopier) **star:211** 结构体拷贝   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [retry](https://github.com/kamilsk/retry) **star:162** 基于上下文的功能机制，反复执行命令直到成功。   ![star > 100][Bronze]
...
* [gubrak](https://github.com/novalagung/gubrak) **star:143** 带有语法糖的Golang实用工具，就像lodash。   ![star > 100][Bronze]
* [gotenv](https://github.com/subosito/gotenv) **star:143** 从 `.env` 或者任何 `io.Reader`。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [util](https://github.com/shomali11/util) **star:137** 有用实用函数的集合。(字符串，并发，操作，…)   ![star > 100][Bronze]
* [robustly](https://github.com/VividCortex/robustly) **star:136** 有弹性的执行函数，遇到错误时捕获并重新运行。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [mimetype](https://github.com/gabriel-vasile/mimetype) **star:126** 用于基于神奇数字的MIME类型检测的包。   ![star > 100][Bronze]
* [chyle](https://github.com/antham/chyle) **star:109** 使用具有多种配置可能性的git存储库生成变更日志。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [ugo](https://github.com/alxrm/ugo) **star:21** uGo 是一个切片工具箱，有着和 Go 语言一致的语法法。   ![最近一年没有更新][Yellow]
...
* [dbt](https://github.com/nikogura/dbt) **star:14** 用于从中心可信存储库运行自更新签名二进制文件的框架。
...
* [structs](https://github.com/PumpkinSeed/structs) **star:13** 简单来讲就是 "Make" 的替代品。   ![最近一年没有更新][Yellow]
* [scan](https://github.com/blockloop/scan) **star:13** 扫描golang的sql。行直接指向结构、片或基本类型。   ![最近一周有更新][Green]
* [filter](https://github.com/gookit/filter) **star:13** 提供Go数据的过滤、清理和转换。
* [ghokin](https://github.com/antham/ghokin) **star:12** 没有外部依赖的gherkin (cucumber, behat…)并行格式化程序。   ![最近一周有更新][Green]
...
* [ulid](https://github.com/oklog/ulid) **star:1688** 实现了ULID(普遍唯一的词典分类标识符)。   ![star > 1000][Silver]
* [uniq](https://gitlab.com/skilstak/code/go/uniq)  没有麻烦，安全，快速的唯一标识符与命令。
* [uuid](https://github.com/gofrs/uuid) **star:570** 通用唯一标识符(UUID)的实现。支持uuid的创建和解析。积极维护satori uuid的fork。   ![star > 100][Bronze]
* [wuid](https://github.com/edwingeng/wuid) **star:291** 一个非常快的唯一数字生成器，比UUID快10-135倍。   ![star > 100][Bronze]
...
* [govalidator](https://github.com/asaskevich/govalidator) **star:3564** 用于字符串，数字，切片和结构的验证器和sanitizers。   ![star > 1000][Silver]
* [validator](https://github.com/go-playground/validator) **star:3558**  Go 结构体及域验证，包括：跨域、跨结构体, Map, 切片和数组。   ![star > 1000][Silver]
* [ozzo-validation](https://github.com/go-ozzo/ozzo-validation) **star:1056** 支持各种数据类型(结构、字符串、映射、片等)的验证，使用可配置和可扩展的验证规则，这些规则在通常的代码构造中指定，而不是在结构标签中指定。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [govalidator](https://github.com/thedevsaddam/govalidator) **star:717** 用简单的规则验证Golang请求数据。深受Laravel请求验证的启发。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [validate](https://github.com/gookit/validate) **star:100**  Go 封装数据验证和过滤。支持验证映射、结构、请求(表单、JSON、url)。值，上载文件)数据和更多特性。   ![star > 100][Bronze]   ![包含中文文档][CN]
...
* [validate](https://github.com/gobuffalo/validate) **star:18** 这个包提供了一个框架，用于为Go应用程序编写验证。
...
* [go-git](https://github.com/src-d/go-git) **star:4311** 纯Go中高度可扩展的Git实现。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [git2go](https://github.com/libgit2/git2go) **star:1368**  libgit2 的 Go 语言接口。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [hercules](https://github.com/src-d/hercules) **star:569** 从Git存储库历史中获得高级见解。   ![star > 100][Bronze]
...
* [goav](https://github.com/giorgisio/goav) **star:798** FFmpeg的Comphrensive。   ![star > 100][Bronze]
* [gmf](https://github.com/3d0c/gmf) **star:536**  FFmpeg av\* 库的 Go 语言接口。   ![star > 100][Bronze]
* [go-astits](https://github.com/asticode/go-astits) **star:260** 在GO中解析和演示MPEG传输流(.ts)。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [go-astisub](https://github.com/asticode/go-astisub) **star:169** 使用 Go 语言操作字幕(.srt, .stl, .ttml, .webvtt, .ssa/.ass, teletext, .smi, etc.)。   ![star > 100][Bronze]
...
* [v4l](https://github.com/korandiz/v4l) **star:29** 用于Linux的视频捕捉库，用Go编写。   ![最近一年没有更新][Yellow]
...
* [Gin](https://github.com/gin-gonic/gin) **star:30298** Gin是一个用Go编写的web框架!它具有一个类似于martini的API，性能更好，速度快40倍。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Beego](https://github.com/astaxie/beego) **star:21565** beego是一种用于 Go 编程语言的开源高性能web框架。   ![star > 5000][Gold]   ![最近一周有更新][Green]   ![包含中文文档][CN]
* [Buffalo](http://gobuffalo.io)  为 Go 语言带来堪比 Rails 的高生产效率!
* [Echo](https://github.com/labstack/echo) **star:14772** 高性能、极简的Go web框架。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Revel](https://github.com/revel/revel) **star:11264** 用于Go语言的高效web框架。   ![star > 5000][Gold]
* [Goa](https://github.com/goadesign/goa) **star:3517** Goa为在Go中开发远程api和微服务提供了一种全面的方法。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-json-rest](https://github.com/ant0ine/go-json-rest) **star:3336** 设置RESTful JSON API的快速简便方法。   ![star > 1000][Silver]
* [Gizmo](https://github.com/NYTimes/gizmo) **star:2854** 《纽约时报》使用的微服务工具包。   ![star > 1000][Silver]
...
* [Air](https://github.com/aofei/air) **star:332** 一个理想的精细化的Go web框架。   ![star > 100][Bronze]
...
* [Aero](https://github.com/aerogo/aero) **star:158** 高性能的Go web框架，在Lighthouse中达到最高分。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [hiboot](https://github.com/hidevopsio/hiboot) **star:87** hiboot是一个高性能的web应用程序框架，支持自动配置和依赖注入。   ![包含中文文档][CN]
...
* [patron](https://github.com/beatlabs/patron) **star:36** Patron是一个遵循最佳云实践的微服务框架，专注于提升开发效率。   ![最近一周有更新][Green]
...
* [rux](https://github.com/gookit/rux) **star:9** 简单而快速的web框架，可用于构建golang HTTP应用程序   ![最近一周有更新][Green]   ![包含中文文档][CN]
...
* [Tollbooth](https://github.com/didip/tollbooth) **star:1265** 限制速率的 HTTP 请求处理程序。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [CORS](https://github.com/rs/cors) **star:1227** 轻松地向API添加CORS功能。   ![star > 1000][Silver]
* [Limiter](https://github.com/ulule/limiter) **star:796** 简单的速度限制中间件。   ![star > 100][Bronze]
* [go-server-timing](https://github.com/mitchellh/go-server-timing) **star:750** 添加/解析Server-Timing头。   ![star > 100][Bronze]
...
* [client-timing](https://github.com/posener/client-timing) **star:14** 用于服务器定时报头的HTTP客户机。

#### 用于创建HTTP中间件的库

* [negroni](https://github.com/urfave/negroni) **star:6350** 符合语言习惯的 HTTP 中间件库。   ![star > 5000][Gold]   ![包含中文文档][CN]
* [alice](https://github.com/justinas/alice) **star:1825** 用于连接中间件的库，简单无痛苦。   ![star > 1000][Silver]
* [render](https://github.com/unrolled/render) **star:1271** Go package用于方便地呈现JSON、XML和HTML模板响应。   ![star > 1000][Silver]
...
* [httprouter](https://github.com/julienschmidt/httprouter) **star:9771** 高性能路由。使用这个库和标准http处理工具可以构建一个非常高性能大web框架。   ![star > 5000][Gold]
* [mux](https://github.com/gorilla/mux) **star:9769** 强大的URL路由器和调度器为golang。   ![star > 5000][Gold]
* [chi](https://github.com/go-chi/chi) **star:6118** 小巧、快速、具有丰富表达力的 HTTP 路由，基于net/context.。   ![star > 5000][Gold]
* [gocraft/web](https://github.com/gocraft/web) **star:1397** Mux和中间件包在Go中。   ![star > 1000][Silver]
...
* [zek](https://github.com/miku/zek) **star:258** 从XML生成Go结构。   ![star > 100][Bronze]
...
* [GoLint](https://github.com/golang/lint) **star:3167** Go 源码的 linter。   ![star > 1000][Silver]
* [errcheck](https://github.com/kisielk/errcheck) **star:1323** Errcheck是一个用于检查Go程序中未检查错误的程序。   ![star > 1000][Silver]
* [gcvis](https://github.com/davecheney/gcvis) **star:920** 实时可视化跟踪 GC 数据。   ![star > 100][Bronze]
* [php-parser](https://github.com/z7zmey/php-parser) **star:647** 用 Go 编写的 PHP 解析器。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [go-critic](https://github.com/go-critic/go-critic) **star:580** 源代码检查工具。   ![star > 100][Bronze]
...
* [go-cleanarch](https://github.com/roblaszczak/go-cleanarch) **star:283** go-cleanarch 的创建是为了验证 Clean 体系结构规则，比如 Go 项目中的依赖关系。   ![star > 100][Bronze]   ![最近一年没有更新][Yellow]
...
* [go-mod-outdated](https://github.com/psampaz/go-mod-outdated) **star:193** 找出项目中过期的依赖项。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [vim-go](https://github.com/fatih/vim-go) **star:10892** Go 开发会用到的 Vim 插件。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [vscode-go](https://github.com/Microsoft/vscode-go) **star:5151** Visual Studio代码的扩展(VS代码)，它提供了对Go语言的支持。   ![star > 5000][Gold]   ![最近一周有更新][Green]
...
* [go-mode](https://github.com/dominikh/go-mode.el) **star:962** 在 GNU/Emacs 支持 GO。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [gotests](https://github.com/cweill/gotests) **star:2219** 从源代码生成测试用例。   ![star > 1000][Silver]
* [genny](https://github.com/cheekybits/genny) **star:979** 优雅的 Go 泛型。   ![star > 100][Bronze]   ![最近一周有更新][Green]
...
* [generic](https://github.com/usk81/generic) **star:29** 灵活的 Go 数据类型。
...
* [go-swagger](https://github.com/go-swagger/go-swagger) **star:4014** 基于 Go 的Swagger 2.0实现。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [go-callvis](https://github.com/TrueFurby/go-callvis) **star:2005** 使用 dot format 可视化 Go 程序的调用图。   ![star > 1000][Silver]
* [richgo](https://github.com/kyoh86/richgo) **star:392** 用文本装饰丰富 go test 的输出。   ![star > 100][Bronze]
...
* [kubernetes](https://github.com/kubernetes/kubernetes) **star:56721** 来自谷歌的容器集群管理器。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Moby](https://github.com/moby/moby) **star:54591** Collaborative project for the container ecosystem to assemble container-based systems.   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [traefik](https://github.com/containous/traefik) **star:23906** 反向代理和负载均衡器，支持多个后端。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [Gitea](https://github.com/go-gitea/gitea) **star:15474** 从 Gogs fork，完全由社区驱动。   ![star > 5000][Gold]   ![最近一周有更新][Green]   ![包含中文文档][CN]
...
* [Vegeta](https://github.com/tsenart/vegeta) **star:12265** HTTP负载测试工具和库。超过9000 !   ![star > 5000][Gold]
* [Hey](https://github.com/rakyll/hey) **star:6382** 压力测试工具，可用来代替 ApacheBench (ab)。   ![star > 5000][Gold]
* [GVM](https://github.com/moovweb/gvm) **star:4498** GVM 提供了一个接口来管理 Go 版本。   ![star > 1000][Silver]
* [Wide](https://wide.b3log.org/login)  为使用 Golang 的团队提供基于 web 的 IDE。
* [webhook](https://github.com/adnanh/webhook) **star:4112** 允许用户创建在服务器上执行命令的 HTTP hooks。   ![star > 1000][Silver]
* [gaia](https://github.com/gaia-pipeline/gaia) **star:3755** 可用于任何编程语言来构建强大的管道。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [gox](https://github.com/mitchellh/gox) **star:3368** 非常简单，没有多余的跨平台编译工具。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [bosun](https://github.com/bosun-monitor/bosun) **star:2858** 按照时间轴发出告警的框架。   ![star > 1000][Silver]
* [bombardier](https://github.com/codesenberg/bombardier) **star:1757** 快速跨平台 HTTP 基准测试工具。   ![star > 1000][Silver]
* [goxc](https://github.com/laher/goxc) **star:1625** 专注于跨平台编译和打包的 Go 构建工具。   ![star > 1000][Silver]   ![最近一年没有更新][Yellow]
* [fac](https://github.com/mkchoi212/fac) **star:1613** 修复 git 合并冲突。   ![star > 1000][Silver]
* [kala](https://github.com/ajvb/kala) **star:1361** 简单、现代和高性能的作业调度程序。   ![star > 1000][Silver]
* [StatusOK](https://github.com/sanathp/statusok) **star:1165** 监视您的网站和REST api。当服务器宕机或响应时间超过预期时，通过Slack、电子邮件获得通知。   ![star > 1000][Silver]
* [s3gof3r](https://github.com/rlmcpherson/s3gof3r) **star:1001** 小型实用程序/库，针对大型对象在Amazon S3中的高速传输进行了优化。   ![star > 1000][Silver]
...
* [Pomerium](https://github.com/pomerium/pomerium) **star:533** Pomerium是一个可识别身份的访问代理。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [aurora](https://github.com/xuri/aurora) **star:406** 基于web的跨平台 Beanstalkd 队列服务器控制台。   ![star > 100][Bronze]
...
* [lstags](https://github.com/ivanilves/lstags) **star:220** 提供了工具和API，可用来同步不同注册中心的Docker图像。   ![star > 100][Bronze]
...
* [DepCharge](https://github.com/centerorbit/depcharge) **star:9** Helps orchestrating the execution of commands across the many dependencies in larger projects.   ![最近一周有更新][Green]
...
* [Seaweed File System](https://github.com/chrislusf/seaweedfs) **star:8302** 快速、简单、可伸缩的分布式文件系统，采用了O(1)磁盘查找。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [rkt](https://github.com/coreos/rkt)  一个应用容器，与其他容器格式(如Docker)兼容，并支持其他执行引擎(如KVM)。
* [restic](https://github.com/restic/restic) **star:7550** 消除重复项备份程序。   ![star > 5000][Gold]   ![最近一周有更新][Green]
* [confd](https://github.com/kelseyhightower/confd) **star:6451** 使用 etcd 或 consul 的模板和数据管理本地应用程序配置文件。   ![star > 5000][Gold]
* [Comcast](https://github.com/tylertreat/Comcast) **star:6180** 模拟坏的网络连接。   ![star > 5000][Gold]
* [LiteIDE](https://github.com/visualfc/liteide) **star:5521** 简单的、开源的、跨平台的Go IDE。   ![star > 5000][Gold]   ![包含中文文档][CN]
...
* [nes](https://github.com/fogleman/nes) **star:4142** 任天堂娱乐系统(NES)模拟器。   ![star > 1000][Silver]
* [toxiproxy](https://github.com/shopify/toxiproxy) **star:3966** 为自动化测试模拟网络和系统条件的代理。   ![star > 1000][Silver]   ![最近一周有更新][Green]
* [tsuru](https://tsuru.io/)  Extensible and open source Platform as a Service software.
* [Pipe](https://github.com/b3log/pipe) **star:3075** 一个小巧漂亮的博客平台。   ![star > 1000][Silver]   ![最近一周有更新][Green]
...
* [GoBoy](https://github.com/Humpheh/goboy) **star:2110** 用 Go 编写的任天堂Game Boy彩色模拟器。   ![star > 1000][Silver]
...
* [lgo](https://github.com/yunabe/lgo) **star:1809** 与 Jupyter 可进行交互 Go 程序。它支持代码完成、代码检查以及与Go 100% 兼容性。   ![star > 1000][Silver]
...
* [Go Package Store](https://github.com/shurcooL/Go-Package-Store) **star:877** App that displays updates for the Go packages in your GOPATH.   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [Documize](https://github.com/documize/community) **star:832** 集成了SaaS工具数据的现代wiki软件。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [scc](https://github.com/boyter/scc) **star:778** 一个非常快速准确的代码计数器，采用了复杂的计算和 COCOMO 预估。   ![star > 100][Bronze]   ![最近一周有更新][Green]
* [Leaps](https://github.com/jeffail/leaps) **star:640** 使用操作转换的成对编程服务。   ![star > 100][Bronze]
* [peg](https://github.com/pointlander/peg) **star:616** 解析表达式语法，是Packrat解析器生成器的实现。   ![star > 100][Bronze]
* [vFlow](https://github.com/VerizonDigital/vflow) **star:602** 高性能、可伸缩和可靠的 IPFIX、sFlow和 Netflow 收集器。   ![star > 100][Bronze]
* [gfile](https://github.com/Antonito/gfile) **star:502** 通过WebRTC在两台计算机之间安全地传输文件，不需要任何第三方依赖。   ![star > 100][Bronze]
* [GoDNS](https://github.com/timothyye/godns) **star:437** 一个动态DNS客户端工具，支持DNSPod & HE.net。   ![star > 100][Bronze]
...
* [DDNS](https://github.com/skibish/ddns) **star:99** 个人 DDNS 客户端。   ![最近一周有更新][Green]
...
* [go-http-routing-benchmark](https://github.com/julienschmidt/go-http-routing-benchmark) **star:1262** HTTP请求路由器基准测试和比较。   ![star > 1000][Silver]
* [go-web-framework-benchmark](https://github.com/smallnest/go-web-framework-benchmark) **star:1010** web框架基准测试。   ![star > 1000][Silver]
...
* [GoBooks](https://github.com/dariubs/GoBooks) **star:6833** 一份精选的 Go 书籍清单。   ![star > 5000][Gold]
...
* [Free Gophers Pack](https://github.com/MariaLetta/free-gophers-pack) **star:1555** 由 Maria Letta 提供的与 Gopher 有关的图片包，其中包含了插图,表情文字。   ![star > 1000][Silver]
...
* [gophers](https://github.com/ashleymcnamara/gophers) **star:1862** 阿什莉·麦克纳马拉的歌斐艺术品。   ![star > 1000][Silver]
...
* [awesome-awesomeness](https://github.com/bayandin/awesome-awesomeness) **star:24691** 其他 awesome 系列的列表。   ![star > 5000][Gold]   ![最近一周有更新][Green]
...
* [Awesome Remote Job](https://github.com/lukasz-madon/awesome-remote-job) **star:14591** Curated list of awesome remote jobs. A lot of them are looking for Go hackers.   ![star > 5000][Gold]   ![最近一周有更新][Green]
...
* [Build web application with Golang](https://github.com/astaxie/build-web-application-with-golang) **star:31706** Golang电子书，主要讲述如何用 Golang 建立一个web应用程序。   ![star > 5000][Gold]   ![包含中文文档][CN]
...
* [Ethereum Development with Go](https://github.com/miguelmota/ethereum-development-with-go-book) **star:458** 一本讲述如何用 Go 进行以太开发的小册。   ![star > 100][Bronze]   ![包含中文文档][CN]
...
* [Learn Go with TDD](https://github.com/quii/learn-go-with-tests) **star:5175** 学习使用测试驱动开发。   ![star > 5000][Gold]   ![最近一周有更新][Green]   ![包含中文文档][CN]
...
* [Golang for Node.js Developers](https://github.com/miguelmota/golang-for-nodejs-developers) **star:702** 引入示例讲述 Golang 与Node.js在学习上的差异。   ![star > 100][Bronze]
...
* [Working with Go](https://github.com/mkaz/working-with-go) **star:1135** 由一个经验丰富的Go程序员群体编写的一系列Go学习范例。   ![star > 1000][Silver]   ![最近一周有更新][Green]
